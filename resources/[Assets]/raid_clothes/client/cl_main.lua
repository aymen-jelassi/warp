...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("warp-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("warp-spawn:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('warp-weapons:getAmmo')
...
RegisterUICallback("warp-ui:raid_clothes:addOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['warp-ui']:openApplication('textbox', {
        callbackUrl = 'warp-ui:raid_clothes:addOutfit',
        header = "Add Outfit",
        key = data.key,
        items = {
            {
...
RegisterUICallback("warp-ui:raid_clothes:addOutfit", function(data, cb)
...
RegisterUICallback("warp-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["warp-ui"]:hideContextMenu()
end)

RegisterUICallback("warp-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            key = slot,
            children = {
                { title = "Change Outfit", action = "warp-ui:raid_clothes:changeOutfit", key = slot},
                { title = "Delete Outfit", action = "warp-ui:raid_clothes:deleteOutfit", key = slot},
            }
        }
    end
    if #menuData >= 0 then
        if #menuData < 10 then
            ...
            menuData[#menuData + 1] = {
                ...
                description = '',
                key = emptySlot,
                action = "warp-ui:raid_clothes:addOutfitPrompt"
            }
        end
...
AddEventHandler("warp-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['warp-textui']:showInteraction(("[M] %s"):format(currentZone.text))
...
RegisterNetEvent("warp-context:closeglobal")
AddEventHandler("warp-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "warp-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }
...
RegisterUICallback("warp-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['warp-ui']:openApplication('textbox', {
        callbackUrl = 'warp-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterUICallback("warp-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    exports['warp-ui']:closeApplication('textbox')
...
RegisterCommand("h1", function(source, args, rawCommand)
    if exports['warp-inventory']:hasEnoughOfItem('hat', 1) then
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports['warp-inventory']:hasEnoughOfItem('mask', 1) then
