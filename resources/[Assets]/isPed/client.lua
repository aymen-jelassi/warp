...
AddEventHandler("warp-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('warp-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('warp-base:removedMoney')
AddEventHandler('warp-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
