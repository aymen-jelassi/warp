...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    ...
    exports['warp-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["warp-inventory"]:hasEnoughOfItem("radio",1,false) or exports["warp-inventory"]:hasEnoughOfItem("civradio",1,false) or exports["warp-inventory"]:hasEnoughOfItem("emsradio",1,false) then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  exports['warp-voice']:setRadioVolume(radioVolume)
  exports['warp-voice']:getRadioVolume()
...
RegisterNetEvent('warp-radio:ResetUI')
AddEventHandler('warp-radio:ResetUI', function()
    GuiOpened = false
    SetNuiFocus(false,false)
    SendNUIMessage({open = false})
    TriggerEvent("animation:radio",GuiOpened)
...
Citizen.CreateThread(function()
  exports["warp-binds"]:registerKeyMapping("", "Radio", "Radio Volume Up", "+RadioVolumeUp", "-RadioVolumeDown", "PAGEUP")

  exports["warp-binds"]:registerKeyMapping("", "Radio", "Radio Volume Down", "+RadioVolumeDown", "-RadiopVolumeDown", "PAGEDOWN")
end)
...
RegisterNetEvent('warp-radio:set_chan_1')
AddEventHandler('warp-radio:set_chan_1', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    pRadioChannel = true
    SendNUIMessage({set = true, setChannel = chan})
...
RegisterNetEvent('warp-radio:set_chan_2')
AddEventHandler('warp-radio:set_chan_2', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    pRadioChannel = true
    SendNUIMessage({set = true, setChannel = chan})
...
RegisterNetEvent('warp-radio:set_chan_3')
AddEventHandler('warp-radio:set_chan_3', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
...
RegisterNetEvent('warp-radio:set_chan_4')
AddEventHandler('warp-radio:set_chan_4', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    exports['warp-voice']:addPlayerToRadio(4)
    exports["warp-voice"]:setVoiceProperty("radioEnabled", true)
    PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
RegisterNetEvent('warp-radio:set_chan_5')
AddEventHandler('warp-radio:set_chan_5', function()
  local job = exports["isPed"]:isPed("myJob")
  if job == "police" or job == "sheriff" or job == "state" then
    exports['warp-voice']:addPlayerToRadio(5)
    exports["warp-voice"]:setVoiceProperty("radioEnabled", true)
    PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
