...
RegisterNetEvent('warp-base:PolyZoneUpdate')
AddEventHandler('warp-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "warp-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('warp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                -- print('warp-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('warp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                -- print('warp-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('warp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                -- print('warp-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('warp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                -- print('warp-polyzone:stop')
            end
        end
