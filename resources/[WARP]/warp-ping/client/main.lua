    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerEvent('warp-hud:hide_ping')
                TriggerServerEvent('warp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('warp-ping:client:SendPing')
AddEventHandler('warp-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('warp-ping:server:SendPingResult', pendingPing.id, 'received')
        TriggerEvent('DoLongHudText', pendingPing.name .. ' Sent You a Ping, Use /ping accept To Accept', 1)
        TriggerEvent('warp-hud:show_ping')
        isPending = true
        ...
        TriggerServerEvent('warp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('warp-ping:client:AcceptPing')
AddEventHandler('warp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerEvent('warp-hud:hide_ping')
        TriggerServerEvent('warp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('warp-ping:client:RejectPing')
AddEventHandler('warp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('warp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('warp-ping:client:RemovePing')
AddEventHandler('warp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("warp-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
