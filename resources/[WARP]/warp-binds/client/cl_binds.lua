-- Key binds:
-- Usage:
-- exports["warp-binds"]:registerKeyMapping(category, description, onKeyDownCommand, onKeyUpCommand, defaultKey)
...
AddEventHandler("warp-binds:should-execute", function(shouldExecute)
...
exports('registerKeyMapping', function(name, category, description, onKeyDownCommand, onKeyUpCommand, default, event, type)
    ...
    RegisterCommand(cmdStringDown, function()
      ...
      if event then TriggerEvent("warp-binds:keyEvent", name, true) end
      ExecuteCommand(onKeyDownCommand)
    end, false)
    RegisterCommand(cmdStringUp, function()
      if not shouldExecuteBind then return end
      if event then TriggerEvent("warp-binds:keyEvent", name, false) end
