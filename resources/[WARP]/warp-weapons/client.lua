...
RegisterNetEvent('warp-weapons:destroyGroundWeapon')
AddEventHandler('warp-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('warp-weapons:allowDrop')
AddEventHandler('warp-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('warp-weapons:dropweapon')
AddEventHandler('warp-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("warp-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('warp-weapons:doHandOver')
AddEventHandler('warp-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('warp-weapons:dropWeaponSearch')
AddEventHandler('warp-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('warp-weapons:giveweapon')
AddEventHandler('warp-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("warp-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["warp-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
