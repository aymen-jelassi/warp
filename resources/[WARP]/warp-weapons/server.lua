RegisterNetEvent('warp-weapons:getAmmo')
AddEventHandler('warp-weapons:getAmmo', function()
    local ammoTable = {}
    local src = source
    ...
    exports.oxmysql:execute("SELECT type, ammo FROM characters_weapons WHERE id = @id", {['id'] = char.id}, function(result)
        ...
        TriggerClientEvent('warp-items:SetAmmo', src, ammoTable)
    end)
end)

RegisterNetEvent('warp-weapons:updateAmmo')
AddEventHandler('warp-weapons:updateAmmo', function(newammo,ammoType,ammoTable)
    local src = source
	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
