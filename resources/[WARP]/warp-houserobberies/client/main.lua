...
Citizen.CreateThread(function()
	local result = RPC.execute('warp-houserobbery:enter_property')
					...
					if #(GetEntityCoords(PlayerPedId()) - objCoords) <= items[i].dist and not items[i].robado then
						...
						if IsControlJustPressed(0,38) and not charging then
							...
							if items[i].anim == 'cajafuerte' then
								...
								TriggerEvent('warp-houserobber:animations',items[i].anim,items[i].delete,i)
							end
						end
					end
				end
...
RegisterNetEvent('warp-houserobbery:getJob')
AddEventHandler('warp-houserobbery:getJob', function()
	if not working then
		if exports['warp-inventory']:hasEnoughOfItem('vpnxj', 1) then
			if exports['warp-inventory']:hasEnoughOfItem('advlockpick', 1) then
				cleanup()
				working = true
				enterCoords = Houses()
...
RegisterNetEvent('warp-houserobbery:make_noise')
AddEventHandler('warp-houserobbery:make_noise', function()
	...
	while stealing do		
		if within then	
			...
			if noise > 100 then
				-- dispatch for pd
				TriggerEvent('warp-dispatch:houserobbery')
				Citizen.Wait(300000)
			end
		end
...
RegisterCommand('raid', function()
	if PlayerJob == "police" or "state" or "sheriff" then
		TriggerEvent('warp-houserobbery:police_enter')	
	end
end)

RegisterNetEvent('warp-houserobbery:police_enter')
AddEventHandler('warp-houserobbery:police_enter', function()
	if PlayerJob == "police" or "state" or "sheriff" then
	...
	while working do
		local espera = 1500
			...
			if distancia < 3 and isnight() then
				espera = 5
				exports['warp-textui']:showInteraction('[H] To Enter House')
					if IsControlJustReleased(0,74) and distancia < 3 and firstentry then
						RemoveBlip(HouseRobberyLocation)
						if exports['warp-inventory']:hasEnoughOfItem('advlockpick', 1) then
							local finished = exports["warp-ui"]:taskBarSkill(5000,math.random(100, 200))
							if (finished == 100) then
								local finished2 = exports["warp-ui"]:taskBarSkill(5000,math.random(200,400))
								if (finished2 == 100) then
									local finished3 = exports["warp-ui"]:taskBarSkill(5000,math.random(50,100))
									if (finished3 == 100) then
										local finished4 = exports["warp-ui"]:taskBarSkill(5000,math.random(200,400))
										if (finished4 == 100) then
											local finished5 = exports["warp-ui"]:taskBarSkill(5000,math.random(200,400))
...
function EnterHouse(police)
	if not stealing then
		...
		stealing = true
		if not police then
			TriggerEvent('warp-houserobbery:make_noise')
		end
	else
...
RegisterNetEvent('warp-houserobber:animations')
AddEventHandler('warp-houserobber:animations', function(anim, delete, i)
	local ped = PlayerPedId()
	...
	elseif anim == 'normal' then
		FreezeEntityPosition(ped,true)	
		AnimBolso()
		TriggerEvent('warp-houserobbery:common_items')
		if delete then
			DeleteEntity(items[i].objeto)
		end
...
function idklol()
	...
	if vehicle then
		...
		exports['warp-textui']:showInteraction('Cannot Place Inside Of Trunk', 'error')
		Citizen.Wait(2500)
		exports['warp-textui']:hideInteraction()
	end
	return false
...
RegisterNetEvent("warp-houserobbery:common_items")
AddEventHandler("warp-houserobbery:common_items", function()
