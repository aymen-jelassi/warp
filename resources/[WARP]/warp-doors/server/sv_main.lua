...
RegisterNetEvent('warp-doors:request-lock-state')
AddEventHandler('warp-doors:request-lock-state', function()
    local src = source
    TriggerClientEvent('warp-doors:initial-lock-state', src, doors)
end)

RegisterNetEvent('warp-doors:change-lock-state')
AddEventHandler('warp-doors:change-lock-state', function(pDoorId, pDoorLockState)
    if doors[pDoorId] then
        doors[pDoorId].lock = pDoorLockState
        TriggerClientEvent('warp-doors:change-lock-state', -1, pDoorId, pDoorLockState)
    end
end)
...
-- RegisterNetEvent("warp-doors:save-config")
-- AddEventHandler("warp-doors:save-config", function(pDoorData)
...
-- RPC.register("warp-doors:elevators:fetch", function()
