	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				local finished = exports["warp-taskbar"]:taskBar(2000,"Making An Exchange")
...
Citizen.CreateThread(function()
	exports["warp-polyzone"]:AddBoxZone("rolex_run_start", vector3(182.79, -1319.47, 29.32), 2, 2, {
...
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "rolex_run_start" then
		...
		if not RolexRun then
			exports["warp-textui"]:showInteraction("[E] Start Run ($500)")
		else 
			exports["warp-textui"]:showInteraction("Finish Your Run")
		end
    end
end)
...
function RolexLocation()
	Citizen.CreateThread(function()
        while RolexSpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				if exports['warp-inventory']:hasEnoughOfItem('rolexwatch', 1) or exports['warp-inventory']:hasEnoughOfItem('stolenpsp', 1) or exports['warp-inventory']:hasEnoughOfItem('stolens8', 1) or exports['warp-inventory']:hasEnoughOfItem('stolenBrokenGoods', 1) or exports['warp-inventory']:hasEnoughOfItem('stolen2ctchain', 1) or exports['warp-inventory']:hasEnoughOfItem('stolenraybans', 1) or exports['warp-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
					if not RolexRun then
