...
RegisterNetEvent("warp-hospitalization:checkin")
AddEventHandler("warp-hospitalization:checkin", function()
	if exports["isPed"]:isPed("countems") <= 0 or exports["isPed"]:isPed("myJob") == 'ems'  then
		...
		local finished = exports["warp-taskbar"]:taskBar(1700,"Checking Credentials")
...
RegisterNetEvent("warp-hospitalization:page")
AddEventHandler("warp-hospitalization:page", function()
	loadAnimDict('anim@narcotics@trash')
	TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
	local finished = exports["warp-taskbar"]:taskBar(1700,"Paging a Doctor")
	if finished == 100 then
...
AddEventHandler("client:bed",function()
  ...
  if DoesEntityExist(objFound) then
	...
	exports['warp-textui']:showInteraction("[E] Leave Bed")
...
function findBed(fadein)
	...
	if not inbed then
		...
		TriggerServerEvent("warp-death:reviveSV2")
	end
end
