...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('warp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('warp-hospital:client:SyncBleed')
AddEventHandler('warp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('warp-hospital:client:FieldTreatLimbs')
AddEventHandler('warp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('warp-hospital:client:FieldTreatBleed')
AddEventHandler('warp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('warp-hospital:client:ReduceBleed')
AddEventHandler('warp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('warp-hospital:client:ResetLimbs')
AddEventHandler('warp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('warp-hospital:client:RemoveBleed')
AddEventHandler('warp-hospital:client:RemoveBleed', function()
    isBleeding = 0
