...
RegisterServerEvent('warp-hospital:server:RequestBed')
AddEventHandler('warp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('warp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('warp-hospital:server:RPRequestBed')
AddEventHandler('warp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('warp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('warp-hospital:server:EnteredBed')
AddEventHandler('warp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('warp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('warp-hospital:server:LeaveBed')
AddEventHandler('warp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)