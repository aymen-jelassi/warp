...
function AlertFight()
    ...
    TriggerServerEvent('warp-dispatch:combatAlert', currentPos)
    if math.random(10) > 5 and not isInVehicle then
        CreateThread(function()
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                TriggerServerEvent('warp-dispatch:combatAlert', newPos)
            end
            return
        end)
    end
...
function AlertGunShot(isHunting, sentWeapon)
    if KnownWeapons[sentWeapon] then
        ...
        TriggerEvent('warp-dispatch:gunshotcl')
        TriggerServerEvent('dispatch:svNotify', {
...
RegisterNetEvent('warp-mdt:drugsale')
AddEventHandler("warp-mdt:drugsale",function()
    TriggerEvent('warp-dispatch:drug')
    DrugSale()
end)
...
function DrugSale()
    ...
    TriggerServerEvent('warp-dispatch:drugsale', currentPos)
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = initialTenCode,
        firstStreet = locationInfo,
        gender = gender,
    ...
    if math.random(10) > 5 and not isInVehicle then
        CreateThread(function()
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                TriggerServerEvent('warp-dispatch:drugsale', newPos)
            end
            return
        end)
    end
...
function CarCrash()
    ...
    TriggerServerEvent('warp-dispatch:vehiclecrash', currentPos)
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
function AlertCheckLockpick(object)
    ...
    TriggerEvent('warp-dispatch:stolenveh')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = initialTenCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-dispatch:houserobbery')
AddEventHandler("warp-dispatch:houserobbery",function()
    AlertHouseRobbery()
end)
...
function AlertHouseRobbery()
    ...
    TriggerEvent('warp-dispatch:robhouse')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
function AlertJewelRob()
    ...
    TriggerServerEvent('warp-dispatch:jewel', currentPos)
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
    ...
    if math.random(10) > 2 and not isInVehicle then
        CreateThread(function()
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                TriggerServerEvent('warp-dispatch:jewel', newPos)
            end
            return
        end)
    end
...
function AlertJailBreak()
    ...
    TriggerServerEvent('warp-dispatch:blip:jailbreak', currentPos)
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
    ...
    if math.random(10) > 2 and not isInVehicle then
        CreateThread(function()
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                TriggerServerEvent('warp-dispatch:blip:jailbreak', newPos)
            end
            return
        end)
    end
...
function AlertFleecaRobbery()
    ...
    local currentVeh = GetVehiclePedIsIn(PlayerPedId(), false)
    local dispatchCode = "10-90"
    TriggerServerEvent('warp-dispatch:bankwobbewy', currentPos)
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
    ...
    if math.random(10) > 2 and not isInVehicle then
        CreateThread(function()
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                TriggerServerEvent('warp-dispatch:bankwobbewy', newPos)
            end
            return
        end)
    end
...
RegisterNetEvent('warp-dispatch:bankrobbery')
AddEventHandler("warp-dispatch:bankrobbery",function()
    AlertFleecaRobbery()
end)

RegisterNetEvent('warp-dispatch:bankrobbery:pacific')
AddEventHandler("warp-dispatch:bankrobbery:pacific",function()
    AlertPacificRobbery()
end)
...
RegisterNetEvent("warp-dispatch:boostTrackerPing")
AddEventHandler("warp-dispatch:boostTrackerPing", function(toggle)
    if toggle == true then
  ...
  TriggerEvent("warp-dispatch:carBoostTracker")
...
function AlertDeath()
    ...
    TriggerEvent('warp-dispatch:downguy')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-dispatch:jewelrobbery')
AddEventHandler("warp-dispatch:jewelrobbery",function()
    AlertJewelRob()
    TriggerEvent('warp-dispatch:jewrob')
end)

RegisterNetEvent('warp-dispatch:carjacking')
AddEventHandler("warp-dispatch:carjacking",function()
    AlertCheckLockpick()
end)
...
LoadDreamsVersion = function()
    ...
    RegisterNetEvent('warp-dispatch:setBlip')
    AddEventHandler('warp-dispatch:setBlip', function(type, pos, id)
        ...
        for i,v in ipairs(gasStations) do
            ...
            if scandst < 20 and scandst < dstcheck then
                ...
                if IsExplosionInSphere(9,v[1],v[2],v[3],60.0)  then
                    ...
                    TriggerEvent('warp-dispatch:gasexplosion')
                    Wait(9000)
                    origin3 = false
                end
            end
...
RegisterNetEvent('warp-dispatch:storerobbery2')
AddEventHandler("warp-dispatch:storerobbery2",function()
    AlertShop()
end)
...
function AlertShop()
    ...
    TriggerEvent('warp-dispatch:robstore')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-dispatch:bank_truck_robbery')
AddEventHandler("warp-dispatch:bank_truck_robbery",function()
    AlertBankTruck()
end)
...
function AlertBankTruck()
    ...
    local currentVeh = GetVehiclePedIsIn(PlayerPedId(), false)
    local dispatchCode = "10-90"
    TriggerEvent('warp-dispatch:bankt')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-dispatch:bobcat_robbery')
AddEventHandler("warp-dispatch:bobcat_robbery",function()
    AlertBobcat()
end)
...
function AlertBobcat()
    ...
    local currentVeh = GetVehiclePedIsIn(PlayerPedId(), false)
    local dispatchCode = "10-90"
    TriggerEvent('warp-dispatch:dispatchBobcat')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-boosting:ping')
AddEventHandler('warp-boosting:ping', function()
    AlertBoosting()
end)
...
RegisterNetEvent('warp-dispatch:unauthorised_aircraft')
AddEventHandler("warp-dispatch:unauthorised_aircraft",function()
    Aircraft()
end)
...
function Aircraft()
    ...
    local currentVeh = GetVehiclePedIsIn(PlayerPedId(), false)
    local dispatchCode = "10-31A"
    TriggerEvent('warp-dispatch:cocaine_plane')
    TriggerServerEvent('dispatch:svNotify', {
        dispatchCode = dispatchCode,
        firstStreet = locationInfo,
        gender = gender,
...
RegisterNetEvent('warp-dispatch:sus_drop')
AddEventHandler("warp-dispatch:sus_drop",function()
    AlertSus()
end)
