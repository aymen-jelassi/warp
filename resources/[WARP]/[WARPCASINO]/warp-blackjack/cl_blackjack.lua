...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(1036.573297, 51.78527328, 69.06023142),
        dealerHeading = 191.0,
        tablePos = vector3(1036.813, 50.90566, 68.06007),  
        tableHeading = 13.932889,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [1] = {
        dealerPos = vector3(1036.071, 54.245925, 69.060165),
        dealerHeading = 9.2,
        tablePos = vector3(1035.8149, 55.130947, 68.08806),
        tableHeading = 195.532,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [2] = {
        dealerPos = vector3(1043.643263, 56.00133277, 69.06022326),
        dealerHeading = 12.424591,
        tablePos = vector3(1043.424, 56.79709, 68.06007),
        tableHeading = 193.465,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [3] = {
        dealerPos = vector3(1044.1833, 53.468566, 69.060203),
        dealerHeading = 191.2901,
        tablePos = vector3(1044.392, 52.66409, 68.06007),
        tableHeading = 13.456861,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
}
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        exports["warp-textui"]:showInteraction("Press [E] to play blackjack", 'info')
                    else 
                        exports["warp-textui"]:showInteraction("This seat is taken")
                        Citizen.Wait(3000)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            local title = "Blackjack " .. "$100.00 / $5000.00"
            local hand = "Your hand : " .. currentHand
            local dealers = "Dealers hand is : " .. dealersHand
            exports["warp-blackjackui"]:sendAppEvent("status-hud", {
              show = true,
              title = title,
              dealers = dealers,
              hand = hand
...
AddEventHandler("Blackjack:successBlackjackBet",function()
    ...
    TriggerEvent("warp-blackjack:close")
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar then
            ...
            if timeLeft == 15 then 
                exports['warp-textui']:showInteraction('Time: '..timeLeft)
                Citizen.Wait(2500)
                exports['warp-textui']:hideInteraction()
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            TriggerEvent("warp-context:closemenu")
        end
    end)
end)

local CurrentChipCount = exports["isPed"]:isPed("mycash")
local RefreshChipCount = true

RegisterNetEvent("betmenu")
AddEventHandler("betmenu", function()
    local tmpInput = exports["warp-applications"]:KeyboardInput({
        header = "Bet Amount",
        rows = {
            {
                id = 1,
...
AddEventHandler("Blackjack:blackjackLose",function(tableId)
    if closeToCasino then
        ...
        angryILost()
        canExitBlackjack = true
        drawNativeNotification("You lose!")
        TriggerEvent("warp-blackjack:close")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
...
RegisterNetEvent("warp-blackjack:close", function()
    exports["warp-blackjackui"]:closeApplication("status-hud", {
      show = false,
      title = title,
      dealers = dealers,
      hand = hand
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("You pushed!")
        TriggerEvent("warp-blackjack:close")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
