...
RegisterServerEvent('warp-pdm:CarTablePDM')
AddEventHandler('warp-pdm:CarTablePDM', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('warp-pdm:ReturnPDMTTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('warp-pdm:FinaceEnabledSV')
AddEventHandler('warp-pdm:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('warp-pdm:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('warp-pdm:BuyEnabledSV')
AddEventHandler('warp-pdm:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('warp-pdm:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('warp-pdm:RequestPDMTTable')
AddEventHandler('warp-pdm:RequestPDMTTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('warp-pdm:ReturnPDMTTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('warp-pdm:ChechMoney')
AddEventHandler('warp-pdm:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('warp-pdm:FailedPurchase', src)
    end
end)

RegisterServerEvent('warp-pdm:BuyVehicle')
AddEventHandler('warp-pdm:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["warp-base"]:getModule("Player"):GetUser(src)
