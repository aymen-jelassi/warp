...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("warp-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("warp-pdm:testdrive")
AddEventHandler("warp-pdm:testdrive", function()
...
RegisterNetEvent("warp-pdm:EnableBuy")
AddEventHandler("warp-pdm:EnableBuy", function()
	...
	TriggerServerEvent("warp-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("warp-pdm:BuyEnabledCL")
AddEventHandler("warp-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("warp-pdm:PDMCommission")
AddEventHandler("warp-pdm:PDMCommission", function(newAmount)
	if exports["isPed"]:GroupRank("pdm") >= 1 or #(vector3(-23.69871, -1094.529, 27.305231) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("warp-pdm:ReturnPDMTTable")
AddEventHandler("warp-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('warp-pdm:ChechMoney',name, model, price)
		...
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["isPed"]:GroupRank("pdm") >= 1 then
				if buyPlate[addplate] ~= nil then
					exports['warp-textui']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['warp-textui']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['warp-textui']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['warp-textui']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("warp-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('warp-pdm:BuyVehicle', plate, name, vehicle, price, VehicleProps)
			DespawnSaleVehicles()
			SpawnSaleVehicles()
			Citizen.Wait(100)
			exports['warp-textui']:hideInteraction()
...
RegisterNetEvent("warp-pdm:FailedPurchase")
AddEventHandler("warp-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "s-class" or this == "a-class" or this == "b-class" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('warp-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
...
RegisterCommand('commission', function(source, args, raw)
	if exports["isPed"]:GroupRank("pdm") >= 1 then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('warp-pdm:PDMCommission', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["isPed"]:GroupRank("pdm") >= 1 then
		TriggerEvent('warp-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["isPed"]:GroupRank("pdm") >= 1 then
		TriggerEvent('warp-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)

RegisterNetEvent('warp-pdm:elevator_roof')
AddEventHandler('warp-pdm:elevator_roof', function()
	if exports["isPed"]:GroupRank("pdm") >= 1 then
		local FinishedPdm = exports['warp-taskbar']:taskBar(5000, 'Calling Elevator')
...
RegisterNetEvent('warp-pdm:elevator_bottom')
AddEventHandler('warp-pdm:elevator_bottom', function()
	if exports["isPed"]:GroupRank("pdm") >= 1 then
		local FinishedPdm1 = exports['warp-taskbar']:taskBar(5000, 'Calling Elevator')
