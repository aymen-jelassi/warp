...
RegisterNetEvent('warp-eblips:server:registerSourceName')
AddEventHandler('warp-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('warp-eblips:server:registerPlayerBlipGroup')
AddEventHandler('warp-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('warp-eblips:server:removePlayerBlipGroup')
AddEventHandler('warp-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('warp-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('warp-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('warp-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('warp-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('warp-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
