			...
			for i = 1, #vehicleWashStation do
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(garageCoords2[1], garageCoords2[2], garageCoords2[3])) < 5 then
					if isCleaning == false then
						exports['warp-textui']:showInteraction("Press [ENTER] to clean your vehicle!")
					else
						exports['warp-textui']:hideInteraction()
					end
					if(IsControlJustPressed(1, Key)) and isCleaning == false then
						TriggerServerEvent('warp-carwash:checkmoney')
					end
				end
			end
...
RegisterNetEvent('warp-carwash:success')
AddEventHandler('warp-carwash:success', function(price)
	TriggerEvent('DoLongHudText', 'Vehicle Clean $' .. price ..'!')
	FreezeEntityPosition(GetVehiclePedIsUsing(PlayerPedId()), true)
	isCleaning = true
	local finished = exports["warp-taskbar"]:taskBar(10000,"Cleaning Vehicle",false,false,playerVeh)
...
RegisterNetEvent('warp-carwash:notenoughmoney')
AddEventHandler('warp-carwash:notenoughmoney', function(moneyleft)
