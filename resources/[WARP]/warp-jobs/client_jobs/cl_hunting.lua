...
Citizen.CreateThread(function()
  while true do
    ...
    if dst < 40 and jobSpawned == false then
      TriggerEvent('warp-hunting:spawnJobPed',dumbass, 225.87)
      jobSpawned = true
      refreshPed = true
    end
    if dst >= 41  then
...
RegisterNetEvent('warp-hunting:spawnJobPed')
AddEventHandler('warp-hunting:spawnJobPed',function(coords, heading)
...
RegisterNetEvent('warp-hunting:spawnAnimal')
AddEventHandler('warp-hunting:spawnAnimal', function()
...
RegisterNetEvent('warp-hunting:skinAnimal')
AddEventHandler('warp-hunting:skinAnimal', function()
  if exports["warp-inventory"]:getQuantity("huntingknife") >= 1 then
    if DoesEntityExist(baitAnimal) then
        LoadAnimDict('amb@medic@standing@kneel@base')
        LoadAnimDict('anim@gangops@facility@servers@bodysearch@')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
        local finished = exports["warp-taskbar"]:taskBar(5000,"Skinning")
        if finished == 100 then
          ClearPedTasksImmediately(PlayerPedId())
          DeleteEntity(baitAnimal)
          TriggerServerEvent('warp-hunting:skinReward')
        end
    end
  else
...
RegisterNetEvent('warp-hunting:usedBait')
AddEventHandler('warp-hunting:usedBait', function()
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(GetPlayerPed(-1),v) then
      if cooldown <= 0 then
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        local finished = exports["warp-taskbar"]:taskBar(5000,"Placing Bait",false,false,playerVeh)
        Citizen.Wait(100)
        cooldown = useBaitCooldown * 10000
        ClearPedTasksImmediately(PlayerPedId())
        used = true
        usedBait()
        TriggerEvent('warp-hunting:spawnAnimal')
        TriggerServerEvent('warp-hunting:removeBait')
        baitCooldown()
      end
    end
  end
...
RegisterNetEvent('dreams-hunting:equip')
AddEventHandler('dreams-hunting:equip', function()
...
local carcasses = {
  { name = "huntingcarcass1", price = 150, illegal = false },
  { name = "huntingcarcass2", price = 250, illegal = false },
  { name = "huntingcarcass3", price = 500, illegal = false },
  { name = "huntingcarcass4", price = 1000, illegal = false },
}
...
local function sellAnimals()
  ...
  for _, carcass in pairs(carcasses) do
      local qty = exports["warp-inventory"]:getQuantity(carcass.name)
...
AddEventHandler("warp-polyzone:enter", function(name)
  ...
  exports['warp-textui']:showInteraction("[E] Sell Animal Carcass")
  listenForKeypress()
end)
...
Citizen.CreateThread(function()
  exports["warp-polyzone"]:AddCircleZone("huntingsales", vector3(12.28, -1299.32, 29.29), 1.3, {
      useZ = true,
      debugPoly= false,
  })
end)