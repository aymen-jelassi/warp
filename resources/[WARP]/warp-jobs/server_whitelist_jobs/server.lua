...
RegisterServerEvent('warp-jobs:union-towing')
AddEventHandler('warp-jobs:union-towing', function(car)
	local src = source
	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    if car == "towtruck" then
        TriggerClientEvent('dreams-jobs:tow-union-bay-2', source, car)
...
RegisterServerEvent('warp-jobs:union-towing-bay-2')
AddEventHandler('warp-jobs:union-towing-bay-2', function(car)
	local src = source
	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    if car == "towtruck" then
        TriggerClientEvent('dreams-jobs:tow-union-bay-1', source, car)
    elseif car == "towtruck2" then
        TriggerClientEvent('dreams-jobs:tow-union-bay-1', source, car)
    elseif car == "flatbed" then
        TriggerClientEvent('dreams-jobs:tow-union-bay-1', source, car)
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["warp-base"]:getModule("Player"):GetUser(Registers[regID][i].owner)
...
RegisterServerEvent('warp-weedbox:payout')
AddEventHandler('warp-weedbox:payout', function(money)
    local source = source
    local player = GetPlayerName(source)
    local user = exports["warp-base"]:getModule("Player"):GetUser(source)
    if money ~= nil then
...
RegisterServerEvent("warp-jobs:cosmic_cannabis:update:pay")
AddEventHandler("warp-jobs:cosmic_cannabis:update:pay", function(cid)
    local src = source
    local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getVar("character").id
    local invname = 'ply-'..characterId
...
RegisterServerEvent('dreams-jobs:spawn-drift-car')
AddEventHandler('dreams-jobs:spawn-drift-car', function(car)
	local src = source
	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    if car == "cookie60" then
        TriggerClientEvent('dreams-jobs:pull-drift-veh', source, car)
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    TriggerEvent("warp_maldinis:update:registers", regID)
                else
                    TriggerClientEvent("DoLongHudText", src, "You cant afford this payment", 2)
                end
            end
...
RegisterServerEvent("warp_maldinis:update:registers")
AddEventHandler("warp_maldinis:update:registers", function(regID)
    for k, v in pairs(Registers[regID]) do
        table.remove(Registers[regID], k)
    end
end)

RegisterServerEvent("warp_maldinis:update:pay")
AddEventHandler("warp_maldinis:update:pay", function(cid)
    local src = source
    local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    local characterId = user:getVar("character").id
    local invname = 'ply-'..characterId
