...
RegisterNetEvent('warp-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('warp-voice:setTalkingOnRadio', setTalkingOnRadio)
...
function addPlayerToRadio(plySource)
	TriggerEvent('warp-voice:getRadioChannel')
...
RegisterNetEvent('warp-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('warp-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('warp-voice:setPlayerRadio', channel)
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
			TriggerServerEvent('warp-voice:setTalkingOnRadio', true)
			radioPressed = true
			playMicClicks(true)
			...
			Citizen.CreateThread(function()
				TriggerEvent('warp-hud:transmitting', radioPressed)
				TriggerEvent("warp-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("warp-voice:radioActive", false)
		TriggerEvent('warp-hud:transmitting', radioPressed)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('warp-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('warp-voice:clSetPlayerRadio', syncRadio)

RegisterNetEvent("warp-voice:getRadioChannel")
AddEventHandler("warp-voice:getRadioChannel", function()
	TriggerServerEvent('warp-dispatch:GrabRadioChannel', radioChannel)
end)