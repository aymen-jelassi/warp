...
AddEventHandler('evidence:dnaSwab', function()
    local finished = exports["warp-taskbar"]:taskBar(8000,"DNA Testing")
...
AddEventHandler('revive', function(t)
	...
	if(distance ~= -1 and distance < 10) then
		TriggerServerEvent("warp-death:reviveSV", GetPlayerServerId(t))
		KneelMedic()
...
-- 		local finished = exports["warp-taskbar"]:taskBar(4000,"Grabbing Scuba Gear")
-- 	  	if finished == 100 then
-- 	  		loadAnimDict('anim@narcotics@trash')
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["warp-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
...
AddEventHandler('police:giveweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:giveLicenses", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removeweaponlicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:removeLicenses", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:givehuntinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:giveLicenseshunt", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removehuntinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:removeLicenseshunt", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:givefishinglicense', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:giveLicensesfish", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:removeLicensesfish', function(t)
	t, distance = GetClosestPlayer()
	if(distance ~= -1 and distance < 7) then
		TriggerServerEvent("warp-base:removeLicensesfish", GetPlayerServerId(t))
	else
		TriggerEvent('DoLongHudText', 'No player near you (maybe get closer)!', 2)
	end
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	if not exports['warp-death']:GetDeathStatus() then
		local taskbar = exports["warp-ui"]:taskBarSkill(math.random(200,300),math.random(5,10))
		if taskbar == 100 then
...
AddEventHandler('police:jailing2', function(pPlayer, time)
	...
	TriggerEvent('warp-base:setEverything')
...
Citizen.CreateThread(function() 
  ...
  while true do
    ...
    if handCuffedWalking or handCuffed then
		...
		DisableControlAction(1, 37, true) --Disables INPUT_SELECT_WEAPON (tab) Actions
		DisablePlayerFiring(PlayerPedId(), true) -- Disable weapon firing
		if (not IsEntityPlayingAnim(PlayerPedId(), "mp_arresting", "idle", 3) and not exports['warp-death']:GetDeathStatus()) or (IsPedRagdoll(PlayerPedId()) and not exports['warp-death']:GetDeathStatus()) then
		...
		if exports['warp-death']:GetDeathStatus() then
			Citizen.Wait(1000)
		end
    end
...
Citizen.CreateThread(function()
	while true do
		...
		if dragging then
			...
			if exports['warp-death']:GetDeathStatus() or IsControlJustPressed(0, 38) or (`WEAPON_UNARMED` ~= GetSelectedPedWeapon(PlayerPedId())) then
				dragging = false
				ClearPedTasks(PlayerPedId())
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["warp-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
	if finished == 100 then
...
function deleteVeh(ent)
	...
	local finished = exports["warp-taskbar"]:taskBar(1000,"Impounding",false,true)	
...
AddEventHandler('impoundVehicle', function()
	...
	TriggerServerEvent("warp-imp:ImpoundCar")
...
AddEventHandler('impoundsc', function()
    ...
    targetVehicle = getVehicleInDirection(coordA, coordB)
	licensePlate = GetVehicleNumberPlateText(targetVehicle)
	TriggerServerEvent("warp-imp:ImpoundCar",licensePlate)
	deleteVeh(targetVehicle)
end)
...
AddEventHandler('police:stealrob', function()
	if not exports['warp-death']:GetDeathStatus() then
		...
		local finished = exports["warp-taskbar"]:taskBar(15000,"Robbing",false,true)	
...
RegisterCommand("serial", function(source, args)
	...
	if dist <= 2.0 then 
		if isPolice or isState or isSheriff then
			...
			exports["warp-taskbar"]:taskBar(20000,"Checking Serial")
			end
		else
...
AddEventHandler('police:tenThirteenA', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' then	
		...
		TriggerEvent('warp-dispatch:1013A')
	end
end)
...
AddEventHandler('police:tenThirteenB', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' then	
		...
		TriggerEvent('warp-dispatch:1013B')
	end
end)
...
AddEventHandler('police:policePanic', function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' then	
		...
		TriggerEvent('warp-dispatch:policepanic')
	end
end)
...
AddEventHandler("police:tenForteenA", function()
	...
	local pos = GetEntityCoords(PlayerPedId(),  true)
		...
		TriggerEvent('warp-dispatch:1014A')
	end
end)
...
AddEventHandler("police:tenForteenB", function()
	...
	if myJob == 'police' or myJob == 'ems' or myJob == 'state' or myJob == 'sheriff' then	
		...
		TriggerEvent('warp-dispatch:1014B')
	end
end)
...
AddEventHandler('ems:panicbutton', function()
	...
	if myJob == 'ems' then
		...
		TriggerEvent('warp-dispatch:emspanic')
	end
end)
...
local isNearDreamsPDEvidence = false

Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("dreams_evidence_spot", vector3(473.95, -994.79, 26.27), 6, 4.5, {
		name="dreams_evidence_spot",
		heading=0,
		--debugPoly=true,
		minZ=24.27,
		maxZ=28.27
...
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "dreams_evidence_spot" then
        local job = exports["isPed"]:isPed("myjob")
        if job == "police" or job == "sheriff" or job == "state" then
            isNearDreamsPDEvidence = true
			exports['warp-textui']:showInteraction('/evidence [number]')
        end
    end
end)
...
RegisterCommand("evidence", function(source, args)
	...
	if job == 'police' or job == 'state' or job == 'sheriff' then
		if isNearDreamsPDEvidence then
...
RegisterNetEvent('dreams-personal-pd-locker')
AddEventHandler('dreams-personal-pd-locker', function()
	local cid = exports["isPed"]:isPed("cid")
	TriggerEvent("server-inventory-open", "1", "personalMRPD-"..cid)
end)

RegisterNetEvent('dreams-pd-options')
AddEventHandler('dreams-pd-options', function()
	TriggerEvent('warp-context:sendMenu', {
		{
			...
			txt = "Personal 🔒",
			params = {
                event = "dreams-personal-pd-locker",
            }
        },
		{
            id = 2,
			...
			txt = "New Drip",
			params = {
                event = "warp-police:clothing_menu",
            }
        },
    })
end)

RegisterNetEvent('warp-police:clothing_menu')
AddEventHandler('warp-police:clothing_menu', function()
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("dreams_mrpd_clothing_or_locker", vector3(461.5, -997.7, 30.69), 4.9, 5.0, {
		name="dreams_mrpd_clothing_or_locker",
		heading=0,
		--debugPoly=true,
		minZ=28.69,
		maxZ=32.69
...
RegisterNetEvent('warp-polyzone:enter')
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "dreams_mrpd_clothing_or_locker" then
        local job = exports["isPed"]:isPed("myjob")
        if job == "police" or job == "sheriff" or job == "state"then
			DreamsPDShit()
            isNearPersonalOrClothing = true
			exports['warp-textui']:showInteraction('[E] Police Changing Room')
        end
    end
end)
...
function DreamsPDShit()
	Citizen.CreateThread(function()
        while isNearPersonalOrClothing do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				TriggerEvent('dreams-pd-options')
			end
		end
	end)
end
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("police_armory_dreams_rp", vector3(482.66, -995.69, 31.67), 2, 1.6, {
        name="police_armory_dreams_rp",
        heading=270,
		--debugPoly=true,
		minZ=27.67,
		maxZ=31.67
...
RegisterNetEvent('warp-polyzone:enter')
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "police_armory_dreams_rp" then
        ArmoryCuzzy = true     
		local myJob = exports["isPed"]:isPed("myJob")
		if myJob == 'police' or myJob == 'state' or myJob == 'sheriff' then
			ArmoryPD()
            exports['warp-textui']:showInteraction("[E] Armory")
        end
    end
end)
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("dreams_state_hq_locker", vector3(361.75, -1593.02, 25.45), 3, 2.6, {
		name="dreams_state_hq_locker",
		heading=320,
		--debugPoly=true,
		minZ=23.45,
		maxZ=27.45
...
RegisterNetEvent('warp-polyzone:enter')
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "dreams_state_hq_locker" then
        local job = exports["isPed"]:isPed("myjob")
        if job == "police" or job == "sheriff" or job == "state"then
			DreamsHQPdShit()
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("police_armory_dreams_rp_2", vector3(364.16, -1603.62, 25.45), 2, 2, {
        name="police_armory_dreams_rp_2",
		heading=320,
		--debugPoly=true,
		minZ=22.65,
		maxZ=26.65
...
RegisterNetEvent('warp-polyzone:enter')
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "police_armory_dreams_rp_2" then
        ArmoryCuzzy2 = true     
		local myJob = exports["isPed"]:isPed("myJob")
		if myJob == 'police' or myJob == 'state' or myJob == 'sheriff' then
			ArmoryPD2()
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("police_armory_sandy", vector3(1862.46, 3690.73, 34.22), 3, 1.1, {
        name="police_armory_dreams_rp_2",
		heading=30,
		--debugPoly=true,
		minZ=32.02,
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("metaldetect_court:1", vector3(241.3, -1079.17, 29.29), 1.0, 1.2, {
	...
	exports["warp-polyzone"]:AddBoxZone("metaldetect_court:2", vector3(245.65, -1079.15, 29.29), 1.0, 1.0, {
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("void_courthouse_guy", vector3(244.88, -1075.6, 29.29), 1, 1, {
...
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "void_courthouse_guy" then
		...
		VoidCourthouse()
		exports['warp-textui']:showInteraction('[E] Collect Items')
    end
end)
...
RegisterNetEvent('warp-police:restore_vehicle')
AddEventHandler('warp-police:restore_vehicle', function()
	local plate = exports["warp-applications"]:KeyboardInput({
	...
	if plate[1] ~= nil then
		TriggerServerEvent('warp-police:restore_car', plate[1].input)
	end
end)

function setPedRestore()
	modelHash = GetHashKey("s_m_y_cop_01")
	RequestModel(modelHash)
	while not HasModelLoaded(modelHash) do
		Wait(1)
	end
	created_ped = CreatePed(0, modelHash , 418.88189, -1024.382, 29.267513  -1, true)
	FreezeEntityPosition(created_ped, true)
	SetEntityHeading(created_ped, 275.83505)
