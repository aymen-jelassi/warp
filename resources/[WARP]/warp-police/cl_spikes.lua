...
AddEventHandler("stress:timedtest",function(alteredValue,scenario)
 ...
 AddEventHandler('police:RifleRack', function()
   ...
   if IsPedInAnyVehicle(PlayerPedId(), false) and exports["isPed"]:isPed("myJob") == 'police' or exports["isPed"]:isPed("myJob") == 'sheriff' or exports["isPed"]:isPed("myJob") == 'state' then
     local finished = exports["warp-taskbar"]:taskBar(1000,"Unlocking rifle rack..",false,true)
     if finished == 100 then
       TriggerEvent("server-inventory-open", "1", "pdrifle-rack"..cid)
       Wait(1000)
     end
   else if not IsPedInAnyVehicle(PlayerPedId(), false) then
     TriggerEvent('DoLongHudText', "Hey Asshole you are not in a vehicle", 2)
   else
     TriggerEvent('DoLongHudText', "Hey Asshole idk if you are a cop or not", 2)
   end
end
 end)

RegisterNetEvent('warp-sandyPD-menu')
AddEventHandler('warp-sandyPD-menu', function()
...
-- Citizen.CreateThread(function()
--     exports["warp-polyzone"]:AddBoxZone("evidence_sandypd", vector3(1850.07, 3694.39, 34.28), 3, 3, {
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("bennys_sandypd", vector3(1878.17, 3695.39, 33.49), 4, 6.8, {
        name="bennys_sandypd",
        heading=30,
        --debugPoly=true,
        minZ=32.09,
...
RegisterNetEvent('warp-police:AttemptBennys:SandyPD')
AddEventHandler('warp-police:AttemptBennys:SandyPD', function()
    local car = GetVehiclePedIsIn(PlayerPedId(), false)
    local myJob = exports["isPed"]:isPed("myJob")
    if myJob == 'police' or myJob == 'state' or myJob == 'sheriff' then

    if car ~= 0 then
        TriggerEvent('event:control:bennys', 16) -- Dev bennys
        else
