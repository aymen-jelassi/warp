...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    events:Trigger("warp-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with warp-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("warp-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["warp-base"]:getModule("Events")
	events:Trigger("warp-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	        ...
	        TriggerEvent("warp-base:firstSpawn")
	    end
    end)
end
...
function Login.setClothingForChar()
    ...
    SetEntityCoords(PlayerPedId(), -8.2153816223145,-582.40881347656,97.819091796875)
    SetEntityHeading(PlayerPedId(),155.90550231934)
    SetGameplayCamRelativeHeading(155.90550231934)
    SetGameplayCamRelativePitch(8.0, 3.0)

    for i=1,25 do
      local posoffset = GetCamCoord(LoginSafe.Cam)
      local setpos = VecLerp(posoffset.x,posoffset.y,posoffset.z, -9.859153, -587.2075, 99.5511, i/200, true)
...
RegisterNetEvent("warp-spawn:finishedClothing")
AddEventHandler("warp-spawn:finishedClothing", function(endType)
    local playerped = PlayerPedId()
    local playerCoords = GetEntityCoords(playerped)
    local pos = vector3(-9.859153, -587.2075, 99.5511)
