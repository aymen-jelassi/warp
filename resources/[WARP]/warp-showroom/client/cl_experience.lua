...
RegisterNUICallback("warp-ui:application-closed", function(data, cb)
...
AddEventHandler("warp-showroom:enterExperience", function()  
  openShowroom("pdm")
  SetNuiFocus(true, true)
end)

RegisterNetEvent('warp-tunercatty')
AddEventHandler('warp-tunercatty', function()
  local rank = exports["isPed"]:GroupRank("tuner_shop")
  if rank >= 1 then
  TriggerEvent('warp-showroom:enterExperience')
else
  TriggerEvent('DoShortHudText', 'You are not tuner shop employee',2)
    end
end)
