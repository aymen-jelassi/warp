function Void.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("warp-base:playerAttemptLogin", src)
    ...
    local function fetchData(_err)
        ...
        Void.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("warp-base:playerLoggedIn", user) TriggerClientEvent("warp-base:playerLoggedIn", src) end
        end)
    end
...
Void.Events:AddEvent(Void.Core, Void.Core.LoginPlayer, "warp-base:loginPlayer")
...
function Void.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	Void.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("warp-base:charactersLoaded", user, data)
			TriggerClientEvent("warp-base:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
Void.Events:AddEvent(Void.Core, Void.Core.FetchPlayerCharacters, "warp-base:fetchPlayerCharacters")
...
Void.Events:AddEvent(Void.Core, Void.Core.CreateCharacter, "warp-base:createCharacter")
...
Void.Events:AddEvent(Void.Core, Void.Core.DeleteCharacter, "warp-base:deleteCharacter")
...
function Void.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('warp-base:setcontrols', src)
	TriggerClientEvent('updatepasses', src)
	TriggerEvent("warp-base:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("warp-base:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
Void.Events:AddEvent(Void.Core, Void.Core.SelectCharacter, "warp-base:selectCharacter")