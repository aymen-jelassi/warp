    ...
    Citizen.CreateThread(function()
        while true do
            if NetworkIsSessionStarted() then
                TriggerEvent("warp-base:playerSessionStarted")
                TriggerServerEvent("warp-base:playerSessionStarted")
                break
            end
        end
    end)
...
RegisterNetEvent("warp-base:waitForExports")
AddEventHandler("warp-base:waitForExports", function()
    if not Void.Core.ExportsReady then return end

    while true do
        Citizen.Wait(0)
        if exports and exports["warp-base"] then
            TriggerEvent("warp-base:exportsReady")
            return
        end
    end
end)
...
function isCharacter(checkType)
    ...
    if checkType == "gender" then
      pass = RPC.execute("warp-base:gender")
    end

    if checkType == "countpolice" then
        pass = RPC.execute("warp:counts", "police")
    end

    if checkType == "countems" then
        pass = RPC.execute("warp:counts", "ems")
    end
