...
local function AddMethod(player)
    ...
    function player.networkVar(self, var, data)
        ...
        TriggerClientEvent("warp-base:networkVar", GetUser(self):getVar("source"), var, data)
    end
    ...
    function player.addMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["warp-base"]:AddLog("Cash Added", GetUser(self), "Money added to user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeMoney(self, amt)
        ...
        Void.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateCash", GetUser(self).source, GetUser(self):getCash(), amt)
                -- exports["warp-base"]:AddLog("Cash Removed", GetUser(self), "Money removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.removeBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                -- exports["warp-base"]:AddLog("Bank Removed", GetUser(self), "Bank removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.addBank(self, amt)
        ...
        Void.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateBalance", GetUser(self).source, GetUser(self):getBalance(), amt)
                -- exports["warp-base"]:AddLog("Bank Added", GetUser(self), "Bank added to user, amount: " .. tostring(amt))
            end
        end)
    end
...
local pos = {}
RegisterServerEvent('warp-base:updatecoords')
AddEventHandler('warp-base:updateCoords', function(x,y,z)
    local src = source
    pos[src] = {x,y,z}
end)
...
AddEventHandler("playerDropped", function(reason)
    ...
    local pUser = exports["warp-base"]:getModule("Player"):GetUser(src)
    ...
    if userjob == "police"or userjob == "sheriff" or userjob == "state" then
        exports['warp-emergencyjobs']:DisconnectJobUpdater(userjob)
        TriggerEvent("warp-emergencyjobs:log:update", src, "clockoff")
    elseif userjob == "ems" then
        exports['warp-emergencyjobs']:DisconnectJobUpdater(userjob)
    end

    Void.Users[src] = nil
    TriggerEvent('warp-base:playerDropped', src, user)
end)