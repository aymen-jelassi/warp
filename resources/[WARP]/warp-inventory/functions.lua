...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, iteminfo, quality)
    ...
    if (itemid == "bento_box_1") then
        local finished = exports['warp-taskbar']:taskBar(1000, 'Unwrapping Bento Box')
        local cid = exports["isPed"]:isPed("cid")
    ...
    if (itemid == "inkedmoneybag") then
        TriggerEvent('warp-illegalactivities:wash_inkedmoneybag')
    end

    if (itemid == "racingipad") then
        TriggerEvent('warp-racing:openGUI')
    end

    if (itemid == "driedbud") then
        if exports['warp-inventory']:hasEnoughOfItem('qualityscales', 1) and exports['warp-inventory']:hasEnoughOfItem('emptybaggies', 1) then
            TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BUM_BIN", 0, true)
            local finished = exports['warp-taskbar']:taskBar(5000, 'Packaging')
            if finished == 100 then
    ...
    if (itemid == "thermal_charge") then
        ...
        TriggerEvent('warp-bobcat:seconddoor')
    ...
    if (itemid == "craftedgemjade") then
        TriggerEvent("cool-buffs:applyBuff", "gemfdsalfkj", { { buff = "strength", percent = 1.0, timeOverride = 120000 * 60 } })
        remove = true
    end

    if (itemid == "craftedgemcitrine") then
        TriggerEvent("cool-buffs:applyBuff", "Ring", { { buff = "jobluck", percent = 0.01, timeOverride = 120000 * 60 } })
        remove = true
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["warp-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pdbadge") then
        ...
        RPC.execute("warp-gov:police:showBadge", ItemInfo.information, pCoord)
    end

    if (itemid == "boostinglaptop") then
        TriggerEvent('warp-boosting:openLaptop')
    end
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["warp-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "weedoz") then
        local finished = exports["warp-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedpackage") then
        if exports['warp-inventory']:hasEnoughOfItem('emptybaggies', 7) then
            local finished = exports['warp-taskbar']:taskBar(5000, 'Preparing')
            if finished == 100 then
    ...
    if (itemid == "key3") then
        TriggerEvent('warp-doors:container_3')
    end
    ...
    if (itemid == "heistlaptop4") then
         TriggerEvent("warp-vault:startvault")
         TriggerEvent('warp-vault:startlowervault')
    end

    if (itemid == "heistlaptop5") then
        TriggerEvent('warp-prachack')
   end

------------- NEW JOINTS FUNCTIONS ~~ Zyloz

if (itemid == "joint" or itemid == "joint2") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)  -- normal joints
    if (finished == 100) then
        Wait(200)
...
if (itemid == "blunt") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)   --- will give you intelegence
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
        TriggerScreenblurFadeOut(100.0)
        TriggerEvent("client:newStress",false, 222)
...
if (itemid == "chiliadkushjoint") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)   --- will give you luck (WIP)
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
        TriggerScreenblurFadeOut(100.0)
        TriggerEvent("client:newStress",false, 120)
        TriggerEvent('luckevent')
        if math.random(100) == 69 then
            TriggerEvent("player:receiveItem","femaleseed",1)
        end
...
if (itemid == "alaskanthunderfuckjoint") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)   --- will give you swim faster
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
        TriggerScreenblurFadeOut(100.0)
        TriggerEvent("client:newStress",false, 130)
...
if (itemid == "cbrownie" or itemid == "cgummies") then
    TriggerEvent("animation:PlayAnimation","pill")
    local finished = exports["warp-taskbar"]:taskBar(3000,"Consuming Edible",false,false,playerVeh)
    if (finished == 100) then
        if exports['warp-inventory']:hasEnoughOfItem('cbrownie', 1) or exports['warp-inventory']:hasEnoughOfItem('cgummies', 1) then
            TriggerEvent("client:newStress",false, 150)
            TriggerEvent('warp-hud:ChangeHunger', -50)
...
if (itemid == "420bar") then
    ...
    local finished = exports["warp-taskbar"]:taskBar(15000,"Consuming Chocolate Bar",false,false,playerVeh)
    if (finished == 100) then
        if exports['warp-inventory']:hasEnoughOfItem('420bar', 1) then
            TriggerEvent("client:newStress",false, 30)
            SetPlayerMaxArmour(PlayerId(), 100) --60
            SetPedArmour( player, 30) --60
...
if (itemid == "yellowjoint") then
    ...
    if (finished == 100) then
        ...
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
        TriggerScreenblurFadeOut(100.0)
        TriggerEvent("client:newStress",false, 200)
        TriggerEvent('warp-hud:show_alert')
        if math.random(100) == 69 then
            TriggerEvent("player:receiveItem","femaleseed",1)
        end
        if math.random(600) == 69 then
...
if (itemid == "bluejoint") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Moon Joint",false,false,playerVeh)  -- normal joints
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
...
if (itemid == "pinkjoint") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Pink Noscar Joint",false,false,playerVeh)  -- normal joints
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
        TriggerScreenblurFadeOut(100.0)
        TriggerEvent("client:newStress",false, 170)
        if math.random(100) == 69 then
            TriggerEvent("player:receiveItem","femaleseed",1)
        end
        if math.random(600) == 69 then
...
if (itemid == "grapejoint") then
    local finished = exports["warp-taskbar"]:taskBar(2000,"Smoking Grape Soda Joint",false,false,playerVeh)  -- normal joints
    if (finished == 100) then
        Wait(200)
        TriggerEvent("animation:PlayAnimation","weed")
        TriggerEvent("stress:timed",1000,"WORLD_HUMAN_SMOKING_POT")
...
-- end of joints
    ...
    if (itemid == "vodka" or itemid == "beer" or itemid == "whiskey" or itemid == "casinovodka" or itemid == "casinorum" or itemid == "wineglass" or itemid == "chglass" or itemid == "pinacolada" or itemid == "martini") then
        ...
        TriggerEvent('warp-hud:ChangeThirst', 50)
    ...
    if (itemid == "coffee") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Drinking Coffee')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('coffee', 1) then
                TriggerEvent('warp-hud:ChangeThirst', 50)
                remove = true
            else
    ...
    if (itemid == "fishtaco" or itemid == "salad" or itemid == "sushiplate") then
        ...
        local finished = exports['warp-taskbar']:taskBar(7000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('fishtaco', 1) or exports['warp-inventory']:hasEnoughOfItem('salad', 1) or exports['warp-inventory']:hasEnoughOfItem('sushiplate', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 50)
                remove = true
            else
    ...
    if (itemid == "fish_taco") then
        TriggerEvent("animation:PlayAnimation","eat")
        TriggerEvent("attachItem", itemid)
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating Taco')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('fish_taco', 1) then
                TriggerEvent("destroyProp")
                remove = true
    ...
    if (itemid == "chocobar") then
        TriggerEvent("animation:PlayAnimation","eat")
        TriggerEvent("attachItem", itemid)
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating Chocolate Bar')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('chocobar', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 12)
                remove = true
            else
                TriggerEvent('destroyProp')
    ...
    if (itemid == "chips") then
        TriggerEvent("animation:PlayAnimation","eat")
        TriggerEvent("attachItem", itemid)
        local finished = exports['warp-taskbar']:taskBar(7000, 'Eating Chips')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('chips', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 15)
                remove = true
            else
                TriggerEvent('destroyProp')
                TriggerEvent('DoLongHudText', 'Oh it fell on then floor ?', 2)
    ...
    if (itemid == "taco" or itemid == "burrito") then
        TriggerEvent("animation:PlayAnimation","eat")
        TriggerEvent("attachItem", itemid)
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('taco', 1) or exports['warp-inventory']:hasEnoughOfItem('burrito', 1) then
                TriggerEvent("destroyProp")
    ...
    if (itemid == "greencow") then
        TriggerEvent("animation:PlayAnimation","drink")
        TriggerEvent("attachItem", itemid)
        local finished = exports['warp-taskbar']:taskBar(6000, 'Drinking')
        if finishied == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('greencow', 1) then
                TriggerEvent('warp-hud:ChangeThirst', 35)
                TriggerEvent('food:Condiment')
                remove = true
            else
                TriggerEvent('destroyProp')
    ...
    if (itemid == "donut" or itemid == "eggsbacon") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('donut', 1) or exports['warp-inventory']:hasEnoughOfItem('eggsbacon', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 30)
                TriggerEvent('food:Condiment')
                remove = true
    ...
    if (itemid == "mshake") then
        ...
        TriggerEvent('warp-hud:ChangeThirst', 45)
    end
    ...
    if (itemid == "Gruppe6Card") then
        TriggerEvent('warp-heists:banktruck:cl')
    end
    ...
    if (itemid == "heavyammo") then
        local finished = exports["warp-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["warp-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
    ...
    if (itemid == "trackerdisabler" ) then
        TriggerEvent('warp-boosting:delayTracker')
        remove = false
    end
    ...
    if (itemid == "pistolammoPD") then
        local finished = exports["warp-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            TriggerEvent("actionbar:ammo",1950175060,50,true)
            remove = true
        end
    ...
    if (itemid == "taserammo") then
        local finished = exports["warp-taskbar"]:taskBar(2000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "empammo") then
        local finished = exports["warp-taskbar"]:taskBar(30000,"Recharging EMP",false,false,playerVeh)
        if (finished == 100) then
   ...
   if (itemid == "driftingkit") then
        ...
        if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
            ExecuteCommand('e mechanic3')
            local finished = exports["warp-taskbar"]:taskBar(20000,"Applying Drifting kit",false,false,playerVeh)
            if finished == 100 then
    ...
    if (itemid == "lightarmor") then
        local finished = exports["warp-taskbar"]:taskBar(3500,"Putting on Armor",true,false,playerVeh)
        local armor = GetPedArmour(PlayerPedId())
        if (finished == 100) then
    ...
    if (itemid == "heavyarmor") then
        local finished = exports["warp-taskbar"]:taskBar(7000,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
            StopAnimTask(PlayerPedId(), 'clothingshirt', 'try_shirt_positive_d', 1.0)
            SetPlayerMaxArmour(PlayerId(), 100) --60
            SetPedArmour( player, 60) --60
    ...
    if (itemid == "pdarmor") then
        local finished = exports["warp-taskbar"]:taskBar(2000,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
            StopAnimTask(PlayerPedId(), 'clothingshirt', 'try_shirt_positive_d', 1.0)
            SetPlayerMaxArmour(PlayerId(), 100) --60
            SetPedArmour( player, 60) --60
    ...
    if (itemid == "bodybag") then
        local finished = exports["warp-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
        local finished = exports["warp-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["warp-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "fishingrod") then
        TriggerEvent("warp-fishing:start-fishing")
    end

    if (itemid == "fishingtacklebox") then
        local finished = exports["warp-taskbar"]:taskBar(5000,"Opening",true,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "nitrous") then
        TriggerEvent("warp-nitro:client:placeNitro")
    end
    ...
    if (itemid == "Gruppe6Card2")  then
        TriggerEvent("warp-heists:bobcat_armed_door")
    end
    ...
    if (itemid == "meth50g") then
        ...
        local finished = exports['warp-taskbar']:taskBar(25000, 'Bagging Meth')
        if finished == 100 then
    ...
    if (itemid == "codenotepad") then
        TriggerServerEvent('warp-heists:srv:rob_safe')
    end

    if (itemid == "registerkey") then
        TriggerServerEvent('warp-heists:srv:rob_register')
    end
    ...
    if (itemid == "coke50g") then
        ...
        local finished = exports['warp-taskbar']:taskBar(25000, 'Cutting coke with baking soda')
        if finished == 100 then
    ...
    if (itemid == "methlabcured") then
        if exports['warp-inventory']:hasEnoughOfItem('drugbaggy', 10) then
            local finished = exports['warp-taskbar']:taskBar(25000, 'Bagging Meth')
            if finished == 100 then
                if exports['warp-inventory']:hasEnoughOfItem('drugbaggy', 10) and exports['warp-inventory']:hasEnoughOfItem('methlabcured', 1) then
    ...
    if (itemid == "sandwich" or itemid == "hamburger" or itemid == "fish_sandwich" or itemid == "salad" or itemid == "salmon_on_rice") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('sandwich', 1) or exports['warp-inventory']:hasEnoughOfItem('hamburger', 1) or exports['warp-inventory']:hasEnoughOfItem('fish_sandwich', 1) or exports['warp-inventory']:hasEnoughOfItem('salad', 1) or exports['warp-inventory']:hasEnoughOfItem('salmon_on_rice', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 30)
                remove = true
            else
    ...
    if (itemid == "skinnedbass" or itemid == "skinnedsalmon" or itemid == "skinnedmarlin" or itemid == "skinnedshark" or itemid == "cookedcrab" or itemid == "cookedshrimp") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('skinnedbass', 1) or exports['warp-inventory']:hasEnoughOfItem('skinnedsalmon', 1) or exports['warp-inventory']:hasEnoughOfItem('skinnedmarlin', 1) or exports['warp-inventory']:hasEnoughOfItem('skinnedshark', 1) or exports['warp-inventory']:hasEnoughOfItem('cookedcrab', 1) or exports['warp-inventory']:hasEnoughOfItem('cookedshrimp', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 46)
                remove = true
            else
                TriggerEvent('DoLongHudText', 'oh shit, it fell on the floor ?', 2)
                TriggerEvent("destroyProp")
    ...
    if (itemid == "cookedsalmon" or itemid == "cookedshark" or itemid == "cookedrice" or itemid == "platedshrimp" or itemid == "platedbass" or itemid == "cookedshrimp") then
        ...
        TriggerEvent("attachItem", itemid)
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('cookedsalmon') or exports['warp-inventory']:hasEnoughOfItem('cookedshark', 1) or exports['warp-inventory']:hasEnoughOfItem('cookedrice', 1) or exports['warp-inventory']:hasEnoughOfItem('platedshrimp', 1) or exports['warp-inventory']:hasEnoughOfItem('platedbase', 1) or exports['warp-inventory']:hasEnoughOfItem('cookedshrimp', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 50)
                remove = true
            else
    ...
    if (itemid == "platedsalmon" or itemid == "platedmarlin") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Eating')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('platedsalmon', 1) or exports['warp-inventory']:hasEnoughOfItem('platedmarlin', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeHunger', 50)
                remove = true
            else
...
AddEventHandler('inv:wellfed', function()
    ...
    TriggerEvent('warp-hud:ChangeHunger', 60)
    TriggerEvent('warp-hud:ChangeThirst', 60)
    burgies = 0
end)
...
AddEventHandler('inv:dogshit', function()
    TriggerEvent("Evidence:StateSet",25,3600)
    TriggerEvent('warp-hud:ChangeHunger', 25)
    TriggerEvent('warp-hud:ChangeThirst', 25)
    burgies = 0
end)
    ...
    if (itemid == "bstoy") then
        local finished = exports['warp-taskbar']:taskBar(6000, 'Opening Toy')
        if finished == 100 then
    ...
    if (itemid == "cola" or itemid == "soda" or itemid == "water") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Drinking')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('cola', 1) or exports['warp-inventory']:hasEnoughOfItem('soda', 1) or exports['warp-inventory']:hasEnoughOfItem('water', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeThirst', 30)
                remove = true
            else
                TriggerEvent('DoLongHudText', 'oh shit, it fell on the floor ?', 2)
                TriggerEvent("destroyProp")
    ...
    if (itemid == "softdrink") then
        ...
        local finished = exports['warp-taskbar']:taskBar(6000, 'Drinking')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('softdrink', 1) then
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeThirst', 50)
                remove = true
            else
    ...
    if itemid == "slushy" then
        ...
        local finished = exports['warp-taskbar']:taskBar(10000, 'Drinking Slushy')
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('slushy', 1) then
                remove = true
                TriggerEvent("destroyProp")
                TriggerEvent('warp-hud:ChangeThirst', 30)
                TriggerEvent('warp-hud:ChangeHunger', 30)
    ...
    TriggerEvent("warp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    TriggerServerEvent("warp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
...
if (itemid == "rpgammo") then
    local finished = exports["warp-taskbar"]:taskBar(30000,"Reloading",false,false,playerVeh)
    if ( finished == 100 and hasEnoughOfItem(itemid, 1, false) ) then
        TriggerEvent("actionbar:ammo",1742569970,1,true)
        remove = true
    end
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh,itemreturn,itemreturnid)
    ...
    if timer > 0 then
        local finished = exports["warp-taskbar"]:taskBar(timer,message,true,false,playerVeh)
  ...
  src = source
    ...
    if exports["isPed"]:isPed("myJob") == 'police' or exports["isPed"]:isPed("myJob") == 'state' or exports["isPed"]:isPed("myJob") == 'ranger' then
        ...
        --     SetVehicleHasBeenOwnedByPlayer(targetVehicle,true)
            ...
            TriggerEvent('animation:lockpickinvtestoutside')
            local finished = exports["warp-taskbar"]:taskBar(10000,"Using Slim Jim")
            if finished == 100 then
...
AddEventHandler('inv:lockPick', function(isForced, inventoryName, slot, itemType)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 and (IsPedAPlayer(driverPed) or not IsPedDeadOrDying(driverPed)) then
            ...
            exports['warp-vehicles']:LockpickVehicleDoor(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
            ...
            Citizen.SetTimeout(math.random(120, 7200) * 1000, function ()
                TriggerServerEvent('voidrp:vehicles:hotPlate', plate, '10-60')
            end)

            if result.success and not isForced then
                exports['warp-flags']:SetVehicleFlag(targetVehicle, 'isStolenVehicle', true)
            end

        lockpicking = false
    else
        if targetVehicle ~= 0 and not isForced then
            if exports["warp-vehicles"]:HasVehicleKey(targetVehicle) then lockpicking = false return end
            ...
            exports['warp-vehicles']:HotwireVehicle(targetVehicle, itemType, isForced, function(result)
                p:resolve(result)
            end)
            ...
            if not result.success then
                print("[DEBUG] [Inventory] Hotwire Result: Failed")
                print("[DEBUG] [Inventory] Hotwire Stage: " .. result.stage)
...
AddEventHandler('InventoryAdvanced:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["warp-ui"]:taskBarSkill(2200,4)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["warp-ui"]:taskBarSkill(math.random(5000,10000),math.random(10,15))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
...
RegisterCommand('+useKeyFob', function()
    if Throttled("warp-doors:doorKeyFob", 1000) then return end
    TriggerEvent("warp-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["warp-binds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
    TriggerServerEvent("inv:playerSpawned")
    TriggerEvent("closeInventoryGui")
end)

RegisterNetEvent('warp-inventory:shitlord-check')
AddEventHandler('warp-inventory:shitlord-check', function()
    if (not hasEnoughOfItem(GetSelectedPedWeapon(PlayerPedId()),1,false)) then
        SetCurrentPedWeapon(PlayerPedId(), `WEAPON_UNARMED`, true)
    end
end)

AddEventHandler("warp-inventory:itemUsed", function(item)
        ...
        while true do
            ...
            if isSmoking then
                if IsPedShooting(ped) then
                    ...
                    TriggerEvent('warp-taskbar_cancel')
                    isSmoking = false
                end
