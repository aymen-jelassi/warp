...
RegisterNetEvent('nightvision:toggle')
AddEventHandler('nightvision:toggle', function()
	local ChangeClothes = true -- Change this to false if you don't want the mask to appear on the players character
	local ped = GetPlayerPed(-1)
		if nv == false then
			nv = true
			TriggerServerEvent("InteractSound_CL:PlayOnOne", "nv", 0.25)
			SetNightvision(true)
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('warp-shops:WeaponShopStatus')
end)
