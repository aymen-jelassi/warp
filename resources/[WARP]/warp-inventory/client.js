...
RegisterNetEvent('warp-base:playerSpawned');
on('warp-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://warp-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('warp-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
async function CloseGui(pIsItemUsed = false) {
    ...
    openedInv = false;
    recentused = [];
    emit('warp-inventory:shitlord-check')
}
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("warp-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("warp-voice:focus:set", false, false, false);}, 1000)
    //   }
}
