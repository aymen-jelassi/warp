...
RegisterNetEvent('warp-items:SetAmmo')
AddEventHandler('warp-items:SetAmmo', function(sentammoTable)
...
--   })
--   TriggerServerEvent("warp-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
  exports["warp-binds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
  RegisterCommand('+actionBar', actionBarDown, false)
  RegisterCommand('-actionBar', actionBarUp, false)

  exports["warp-binds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
...
AddEventHandler('warp-actionbar:hotreload', function()
	...
	while true do
    	...
    	if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["warp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["warp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("warp-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID)
	--GiveAmmoNow()
	-- if not exports["warp-propattach"]:canPullWeaponHoldingEntity() then return end
	...
	if armed then
		...
		--TriggerEvent('warp-hud:hide_weapon_out')
		holster1h()		
	else
		armed = true
		TriggerEvent("hud-display-item",tonumber(hash),"Equip")
		--TriggerEvent('warp-hud:show_weapon_out')
	...
	TriggerEvent("warp-weapons:client:showWeaponFireRate", armed)
end)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("warp-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('warp-item:CheckClientAmmo')
AddEventHandler('warp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["warp-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["warp-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["warp-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["warp-inventory"]:hasEnoughOfItem("weaponscope", 1, false, true) then
	...
	if exports["warp-inventory"]:hasEnoughOfItem("weaponuziextended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["warp-inventory"]:hasEnoughOfItem("weaponuzifoldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["warp-inventory"]:hasEnoughOfItem("weaponuziwoodstock", 1, false, true) then
