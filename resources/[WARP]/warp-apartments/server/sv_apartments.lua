...
AddEventHandler("apartment:serverUnlockAprts",function(roomType)
    ...
    exports.oxmysql:execute("select * from character_motel WHERE cid=@cid",{
        ...
        if #result > 0 then
            if result[1].building_type == roomType then
                TriggerClientEvent("warp-apps:unlocks1",src)
            else
                TriggerClientEvent("warp-apps:unlocks2",src)
            end
        end
    end)
end)
...
RegisterServerEvent('warp-apartments:SetNewRouting')
AddEventHandler('warp-apartments:SetNewRouting', function(roomNumber)
    local src = source
    SetPlayerRoutingBucket(src, roomNumber)
end)

RegisterServerEvent('warp-apartments:GetRouting')
AddEventHandler('warp-apartments:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('warp-apartments:SetOldRouting')
AddEventHandler('warp-apartments:SetOldRouting', function()
    local src = source
