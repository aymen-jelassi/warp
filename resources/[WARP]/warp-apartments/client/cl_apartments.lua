...
RegisterNetEvent('warp-binds:keyEvent')
AddEventHandler('warp-binds:keyEvent', function(name,onDown)
...
function Apart.enterMotel(roomNumber,roomType,isSpawn)
    ...
    TriggerServerEvent('warp-apartments:SetNewRouting', roomNumber)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    DoScreenFadeOut(1)
    TriggerServerEvent('warp-apartments:GetRouting')
    TriggerEvent("inhotel",true)
    local name = ""
    name = Apart.FindCurrentRoom(roomType)

    local isBuiltCoords = exports["warp-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            Apart.WakeFromBed()
            TriggerEvent("warp-spawn:characterSpawned")
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["warp-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["warp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["warp-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
    ...
    TriggerServerEvent('warp-apartments:SetOldRouting')
    Citizen.Wait(100)
    TriggerServerEvent('warp-apartments:GetRouting')
end
...
--     exports["warp-build"]:getModule("func").CleanUpArea()
--     local cid = exports["isPed"]:isPed("cid")
--     TriggerServerEvent("warp-jobmanager:onCharSwap", cid)
--     Citizen.Wait(1000)   
...
function garageToWorld()
    ...
    exports["warp-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["warp-build"]:getModule("func").getCurrentBed()
...
function nearClothing()
    ...
    local isNear = exports["warp-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["warp-build"]:getModule("func").isInBuilding() then 
...
RegisterUICallback("warp-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        -- exports["warp-ui"]:showContextMenu(MenuData["apartment_check"])
    end
end)
