...
RegisterNetEvent('warp-meth:place_table')
AddEventHandler('warp-meth:place_table', function()
    PlacedTable = true
    TriggerEvent('inventory:removeItem', 'methtable', 1)
    TriggerEvent("warp-drugs:place-meth", `bkr_prop_meth_table01a`)
end)

RegisterNetEvent("warp-drugs:place-meth-bench")
AddEventHandler("warp-drugs:place-meth-bench", function(model, coords)
    RequestModel(model)
    CreatedObjects = CreateObject(model, coords)
    FreezeEntityPosition(CreatedObjects, true)
    TriggerServerEvent("warp-drugs:new-meth-bench", model, coords)
end)
...
local hidden = false
scenes = {}
local DreamsDrugsMethB = false
local coords = {}

RegisterNetEvent("warp-drugs:place-meth")
AddEventHandler("warp-drugs:place-meth", function(model)
    exports['warp-textui']:showInteraction('[E] Confirm Placement')
    local placement = MethTarget()
    coords = {}
    DreamsDrugsMethB = true

    while DreamsDrugsMethB do
        RequestModel(model)
        ...
        if IsControlJustReleased(0, 46) then
            ...
            local finished = exports['warp-taskbar']:taskBar(7000, 'Placing')
            if finished == 100 then
                TriggerEvent("warp-drugs:place-meth-bench", model, placement)
...
RegisterNetEvent('warp-meth:start_cook')
AddEventHandler('warp-meth:start_cook', function()
    if PlacedTable and not CooldownCook then
        StartedCook = true
        -- Minigame
        TriggerEvent('DoLongHudText', 'Cook Started', 1)
    else
        print('[DEBUG] - [warp] - WAIT 1 HOUR COOLDOWN PER METH COOK!')
    end
end)

-- Prepare Good interact 3rd eye

RegisterNetEvent('warp-meth:prepare-goods')
AddEventHandler('warp-meth:prepare-goods', function()
    if StartedCook and not PreparedGoods then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['warp-taskbar']:taskBar(30000, 'Preparing Goods')
        if finished == 100 then
...
RegisterNetEvent('warp-meth:combine-goods')
AddEventHandler('warp-meth:combine-goods', function()
    if StartedCook and PreparedGoods and not CombinedGoods then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","type")
        local finished = exports['warp-taskbar']:taskBar(60000,'Combine Goods')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            CombinedGoods = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('warp-meth:mix_in_water')
AddEventHandler('warp-meth:mix_in_water', function()
    if StartedCook and PreparedGoods and CombinedGoods and not MixedInWater then
        ...
        Wait(50)
        TaskPlayAnim(GetPlayerPed(-1),"weapon@w_sp_jerrycan","fire",2.0, -8, -1,49, 0, 0, 0, 0)
        local finished = exports['warp-taskbar']:taskBar(45000,'Mix In Water')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            MixedInWater = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('warp-meth:add_solvent')
AddEventHandler('warp-meth:add_solvent', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and not AddedSolvent then
        ...
        Wait(50)
        TaskPlayAnim(GetPlayerPed(-1),"weapon@w_sp_jerrycan","fire",2.0, -8, -1,49, 0, 0, 0, 0)
        local finished = exports['warp-taskbar']:taskBar(50000,'Adding Solvent')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
...
RegisterNetEvent('warp-meth:crystalize_product')
AddEventHandler('warp-meth:crystalize_product', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and AddedSolvent and not Crystalized then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['warp-taskbar']:taskBar(120000,'Crystalizing Product')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            Crystalized = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
...
RegisterNetEvent('warp-meth:pack-product')
AddEventHandler('warp-meth:pack-product', function()
    if StartedCook and PreparedGoods and CombinedGoods and MixedInWater and AddedSolvent and Crystalized and not Packed then
        FreezeEntityPosition(GetPlayerPed(-1), true)
        TriggerEvent("animation:PlayAnimation","browse")
        local finished = exports['warp-taskbar']:taskBar(170000,'Packing Product')
        if finished == 100 then
            FreezeEntityPosition(GetPlayerPed(-1), false)
            Packed = true
            TriggerEvent('DoLongHudText', 'Step completed.', 1)
            TriggerEvent('player:receiveItem', 'methlabcured', 1)
            TriggerEvent('warp-meth:cooldown')
        else
            FreezeEntityPosition(GetPlayerPed(-1), false)
        end
    end
...
RegisterNetEvent('warp-methtable:pick_up')
AddEventHandler('warp-methtable:pick_up', function()
    if PlacedTable then
        TriggerEvent("animation:PlayAnimation","mechanic3")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['warp-taskbar']:taskBar(10000 ,'Picking Up Table')
        if finished == 100 then
...
RegisterNetEvent('warp-meth:cooldown')
AddEventHandler('warp-meth:cooldown', function()
    CooldownCook = true
    Citizen.Wait(3.6e+6)
    StartedCook = false
    PreparedGoods = false
    ...
    data = {
        {
            ...
            icon = "temperature-high",
            event = "warp-meth:crystalize_product",
            parameters = {}
        }
    },
    options = {
