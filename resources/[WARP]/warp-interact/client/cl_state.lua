...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags['isJobEmployer'] then
        pContext.job = exports['warp-jobs']:GetNPCJobData(npcId)
    end
end
...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports['warp-flags']:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        --context.meta = exports['warp-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['warp-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports['warp-flags']:GetObjectFlags(pEntity)
    end
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports['warp-vehicles']:HasVehicleKey(pEntity)
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('mizrp-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("warp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['warp-vehicledegrade']:GetVehicleIdentifier(pEntity)
...
RegisterNetEvent('warp-jobs:jobChanged')
AddEventHandler('warp-jobs:jobChanged', function(pJobId)
    CurrentJob = pJobId
...
AddEventHandler("warp-polyzone:enter", function(zone, data)
    if zone == "warp-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
AddEventHandler("mizrp-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("warp-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("warp-interact:grabDrink", function(args, entity, context)
