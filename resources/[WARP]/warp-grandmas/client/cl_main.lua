...
Citizen.CreateThread(function()
  exports["warp-polyzone"]:AddBoxZone("grandmas", vector3(2435.32, 4966.31, 42.35), 2.8, 2, {
    name="grandmas",
    heading=315,
    --debugPoly=true,
    minZ=40.55,
...
AddEventHandler('warp-polyzone:enter', function(name)
  if name == "grandmas" then
      NearGrandmas = true
      NearGrandmasHoe()
      TriggerEvent('warp-textui:ShowUI', 'show', ("%s"):format("[E] - Grandmas ($1000)")) 
    end
end)
...
AddEventHandler('warp-polyzone:exit', function(name)
  ...
  TriggerEvent('warp-textui:HideUI')
end)
...
RegisterNetEvent("grandmas:success:attemt", function()
  local finished = exports["warp-taskbar"]:taskBar(math.random(60000, 120000), "Grandma is helping you up!")
  if finished == 100 then
    TriggerEvent("warp-death:revive")
