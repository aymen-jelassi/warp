Citizen.CreateThread(function()
  ...
  AddEventHandler('phone:EndCall', function(mySourceID, stupidcallnumberidk, somethingextra)
    ...
    TriggerClientEvent('warp-phone:RemoveCall', src)
  ...
  RegisterServerEvent('server:givepass2')
  AddEventHandler('server:givepass2', function(pass_type, pass_name, rank, cid)
    local src = source
    local user = exports["warp-base"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
        ...
        if not data[1] then
            exports.oxmysql:execute("SELECT * FROM characters WHERE id = ?", {cid}, function(name)
                if name[1] then
                    ...
                    TriggerClientEvent('phone:don:getStuff', {GroupID=pass_type})
                else
  ...
  RegisterServerEvent("warp-phone:grabWallpaper")
  AddEventHandler("warp-phone:grabWallpaper", function()
    local src = source
    local player = exports["warp-base"]:getModule("Player"):GetUser(src)
    local char = player:getCurrentCharacter()
    local cid = char.id
      exports.oxmysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
        if data[1].wallpaper ~= nil then
            TriggerClientEvent('warp-phone:grabBackground', src, data[1].wallpaper)
        else
            TriggerClientEvent('warp-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        end
      end)
  end)
  
  RegisterNetEvent('warp-phone:twatSendNotification')
  AddEventHandler('warp-phone:twatSendNotification', function(handle, twat, time)
        TriggerClientEvent('warp-phone:TwatNotify', -1, handle, twat, time)
  end)