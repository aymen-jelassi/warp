    ...
    window.addEventListener('message', function (event) {
        ...
        switch (item.openSection) {
            ...
            case "newsms":
                ...
                break;
            case "phonemedio":
                $(".jss13").removeClass("slideout").addClass('slidein').show().css("bottom" , "-550px").add($('.jss13')).fadeIn();
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://warp-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$('.garage-container-entries-wrapper').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://warp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
...
$("#twat-gifword").change(function() {
   ...
   $(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://warp-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://warp-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
$(document).ready( function() {
    $("#pinger_button").click(function() {
        let id = $("#pingerid").val()
        $.post('https://warp-phone/pingSend', JSON.stringify({
            number: id,
        }));
    })
})
...
function SetWaypointPostOP() {
    $.post('http://warp-phone/SetPostOpNewWaypoint', JSON.stringify({}));
}

function SetWaypointFarming() {
    $.post('http://warp-phone/SetFarmerJobWaypoint', JSON.stringify({}));
}

function SetWaypointFarmingSales() {
    $.post('http://warp-phone/SetFarmerJobWaypointSale', JSON.stringify({}));
}

function SetWaypointFarmingProcess() {
    $.post('http://warp-phone/SetFarmerJobWaypointProcess', JSON.stringify({}));
}

function SetWaypointMining() {
    $.post('http://warp-phone/SetMiningNewWaypoint', JSON.stringify({}));
}

function SetWaypointWaterNPower() {
    $.post('http://warp-phone/SetWaterNPowerNewWaypoint', JSON.stringify({}));
}

function SetWayPointGarbage() {
    $.post('http://warp-phone/SetGarbageNewWaypoint', JSON.stringify({}));
}

function SetWayPointChickenJob() {
    $.post('http://warp-phone/SetChickenNewWaypoint', JSON.stringify({}));
}

function SetWayPointFishingJob() {
    $.post('http://warp-phone/SetFishingNewWaypoint', JSON.stringify({}));
}

function SetWayPointHunting() {
    $.post('http://warp-phone/SetHuntingNewWaypoint', JSON.stringify({}));
}

// Sale Locations

function MiningSales() {
    $.post('http://warp-phone/SetMiningSalesLocation', JSON.stringify({}));
}

function ChickenSales() {
    $.post('http://warp-phone/SetChickenSalesLocation', JSON.stringify({}));
}

function FishingSales() {
    $.post('http://warp-phone/SetFishingSalesLocation', JSON.stringify({}));
}

function HuntingSales() {
    $.post('http://warp-phone/SetHuntingSalesLocation', JSON.stringify({}));
}
