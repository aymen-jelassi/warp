...
function correct(){
    ...
    $.post('https://warp-ui/warp-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const CloseMenu = () => {
    ...
    saved = "";
    textboxactive = false
    $.post(`https://warp-ui/guccimanecancel`)
...
const PostData = (data) => {
    return $.post(`https://warp-ui/`+textmenucallbackurl, JSON.stringify(data))
}

function closesettings(){
    $(".settings").hide();
    $.post('https://warp-ui/close');
}
                    ...
                    $.each(payload.options, function (index2, item2) {
                        if(index1 == index2 && item2 == true){
                            ...
                            $("#target-"+index1).hover((e)=> {
                                $("#target-"+index1).css("color",e.type === "mouseenter"?"#E95AD7":"white")  
                            })
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://warp-ui/warp-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        if (contextmenuactive) {
            ...
            $(".buttoncontext").html("");
            $(".containercontext").hide();
            $.post('https://warp-ui/closecontext', JSON.stringify({}));
        }
        if (textboxactive) {
            CancelMenu();
        }
        if (eyeintactive){
            eyeintactive = false
            $.post('https://warp-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $(".buttoncontext").html("");
                    $(".containercontext").hide();
                    $.post('https://warp-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://warp-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
