local defaultHash, defaultHash2, defaultHash3, defaultHash4, defaultHash5, defaultHash6, defaultHash7 = "npolvette","npolstang", "npolchal","npolmm", "npolchar", "chr20"

local pursuitEnabled = false
local InPursuitModeAPlus = false
local InPursuitModeB = false

RegisterNetEvent("warp-hud:pursuit_mode_a")
AddEventHandler("warp-hud:pursuit_mode_a",function()
...
RegisterNetEvent("warp-hud:pursuit_mode_b") -- Second Pursuit Mode
AddEventHandler("warp-hud:pursuit_mode_b",function()
	local ped = PlayerPedId()
	if (IsPedInAnyVehicle(PlayerPedId(), true)) then
		local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
		local Driver = GetPedInVehicleSeat(veh, -1)
...
RegisterNetEvent("warp-hud:pursuit_mode_s")  -- Final Pursuit Mode for now anyways 0_o
AddEventHandler("warp-hud:pursuit_mode_s",function()
    local ped = PlayerPedId()
	if (IsPedInAnyVehicle(PlayerPedId(), true)) then
		local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
		local Driver = GetPedInVehicleSeat(veh, -1)
...
RegisterNetEvent("warp-hud:pursuit_mode_off")
AddEventHandler("warp-hud:pursuit_mode_off",function()
local ped = PlayerPedId()
if (IsPedInAnyVehicle(PlayerPedId(), true)) then
	local veh = GetVehiclePedIsIn(PlayerPedId(),false)  
	local Driver = GetPedInVehicleSeat(veh, -1)
