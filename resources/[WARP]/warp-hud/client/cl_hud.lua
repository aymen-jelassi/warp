...
RegisterNetEvent('warp-hud:show_money')
AddEventHandler('warp-hud:show_money', function()
  isLooting = 1
...
RegisterNetEvent('warp-hud:show_alert')
AddEventHandler('warp-hud:show_alert', function()
  alerttime = 1
...
AddEventHandler('hadsugar', function()
  ...
  drugEffectTime = 0
  TriggerEvent('warp-hud:hidedrugs')
...
AddEventHandler('inventory:ketamine', function(pPurity)
  ...
  TriggerServerEvent("warp-drugeffects:forceStress", 1, 3 * 60 * 1000)
  ...
  ketaActive = false
  TriggerEvent("fx:run", "cocaine", 10, 0.0, false, false)
  TriggerServerEvent("warp-drugeffects:forceStress", 5000, 10 * 60 * 1000)
end)
...
RegisterNetEvent("warp-hud:EnableHud")
AddEventHandler("warp-hud:EnableHud", function(status)
    isLoggedIn = status
end)
		...
		if not IsPedSwimmingUnderWater( PlayerPedId() ) and currentValues["oxy"] < 25.0 then
			if GetGameTimer() - lastDamageTrigger > 3000 then
				...
				if currentValues["oxy"] <= 0 then
                    local dead = exports['warp-death']:GetDeathStatus()
...
RegisterNetEvent('warp-admin:currentDevmode')
AddEventHandler('warp-admin:currentDevmode', function(test)
    developer = test
end)

RegisterNetEvent('warp-admin:currentDebug')
AddEventHandler('warp-admin:currentDebug', function(status)
    debug = status
end)
...
RegisterNetEvent("warp-hud:changeRange")
AddEventHandler("warp-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
Citizen.CreateThread(function()
    while true do
	    if exports['warp-hud']:BuffHunger() and currentValues["hunger"] > 0 then
      currentValues["hunger"] = currentValues["hunger"] - math.random(2)
        else
        currentValues["hunger"] = currentValues["hunger"] - math.random(3)
end
	    if exports['warp-hud']:BuffThirst() and currentValues["thirst"] > 0 then
...
RegisterNetEvent('warp-hud:ChangeThirst')
AddEventHandler('warp-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('warp-hud:ChangeHunger')
AddEventHandler('warp-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterCommand('hud', function()
    TriggerEvent('warp-hud:open-hud')
end)

RegisterNetEvent('warp-bruh:pay')
AddEventHandler('warp-bruh:pay', function()
    local randomMoney = math.random(340, 500)
    user:addMoney(randomMoney)
end)

RegisterNetEvent('warp-hud:open-hud')
AddEventHandler('warp-hud:open-hud', function()
        ...
        if (IsPedInAnyVehicle(player, false)) then
            ...
            if GetPedInVehicleSeat(veh, -1) == player then
                ...
                if ( setLastUpdate > 300) then
                  exports["warp-vehicles"]:SetVehicleFuel(veh, fRound(Fuel))
                  lasteupdate = 0
                end
            ...
            if DivingStatus then
                DivingStatus = false
                exports["warp-vehicles"]:SetVehicleFuel(updateLatestVehicle, fRound(Fuel))
            end
            Citizen.Wait(1500)
        end
    end
...
RegisterNetEvent('warp-gas:checkpump')
AddEventHandler('warp-gas:checkpump', function()
    local ped = PlayerPedId()
    ...
    if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
        vehicleCurrentFuel = exports["warp-vehicles"]:CurrentFuel()
...
exports("setFuel", function(veh, amt)
  exports["warp-vehicles"]:SetVehicleFuel(veh, amt)
end)
...
RegisterNetEvent("warp-admin:currentDevmode")
AddEventHandler("warp-admin:currentDevmode", function(devmode)
    isBlocked = devmode
end)

RegisterNetEvent("warp-hud:updateStress")
AddEventHandler("warp-hud:updateStress", function(status, amount)
    if status == true then
        pStress = pStress + amount
        TriggerEvent("DoShortHudText",'Stress Gained',6)
        Wait(1000)
        TriggerServerEvent('warp-hud:UpdateStress_SV', pStress)
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('warp-hud:EnableHud', false)
    TriggerScreenblurFadeOut(100.0)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('warp-banking:ResetUI')
    TriggerEvent('warp-radio:ResetUI')
    TriggerEvent('warp-menu:ResetUI')
    TriggerEvent('warp-interact:ResetUI')
    TriggerEvent('warp-mdt:ResetUI')
    TriggerEvent('warp-ui:restartUI')
    TriggerEvent('menu:menuexit')
    TriggerEvent('clothing:close')
    TriggerServerEvent('void:getmapprefrence')
    exports['warp-textui']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['warp-textui']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['warp-textui']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['warp-textui']:showInteraction("UI Successfully Restarted")
    Wait(1000)
...
RegisterCommand("anim-r", function()
    exports["warp-textui"]:showInteraction("Reseting Animations .")
    Wait(1000)
    exports["warp-textui"]:showInteraction("Reseting Animations ..")
    Wait(1000)
    exports["warp-textui"]:showInteraction("Reseting Animations ...")
    ...
    ClewarpedTasks(PlayerPedId())
    exports["warp-textui"]:showInteraction("Animations Restarted !", "success")
    Wait(1000)
    exports["warp-textui"]:hideInteraction()
...
function removeAttachedProp2()
  ...
  RegisterNetEvent("warp-hud:jerry_can:refuel")
  AddEventHandler("warp-hud:jerry_can:refuel",function()
      vehicleCurrentFuel = exports["warp-vehicles"]:CurrentFuel(veh) --DecorGetInt(veh, "GetVehicleCurrentFuel")
      if exports['warp-inventory']:hasEnoughOfItem('883325847', 1) then
        ...
        if finished == 100 then
            if exports['warp-inventory']:hasEnoughOfItem('883325847', 1) then
                --DecorSetInt(veh, "GetVehicleCurrentFuel", 100)
                exports["warp-vehicles"]:SetVehicleFuel(veh, 100)
  ...
  RegisterNetEvent('warp-hud:show_weapon_out')
  AddEventHandler('warp-hud:show_weapon_out', function()
    HasWeaponOut = 100
    SendNUIMessage({action = "shoot", shoot = HasWeaponOut})
  end)

  RegisterNetEvent('warp-hud:hide_weapon_out')
  AddEventHandler('warp-hud:hide_weapon_out', function()
    HasWeaponOut = 0
    SendNUIMessage({action = "shoot", shoot = HasWeaponOut})
  end)
  ...
  RegisterNetEvent('warp-hud:show_ping')
  AddEventHandler('warp-hud:show_ping', function()
    ecompass = 100
    SendNUIMessage({action = "ecompass", ecompass = ecompass})
  end)

  RegisterNetEvent('warp-hud:hide_ping')
  AddEventHandler('warp-hud:hide_ping', function()
    ecompass = 0
    SendNUIMessage({action = "ecompass", ecompass = ecompass})
  end)

  RegisterNetEvent('warp-hud:show_hackerman')
  AddEventHandler('warp-hud:show_hackerman', function()
    chipmode = 100
    SendNUIMessage({action = "chipmode", chipmode = chipmode})
  end)

  RegisterNetEvent('warp-hud:hide_hackerman')
  AddEventHandler('warp-hud:hide_hackerman', function()
    chipmode = 0
    SendNUIMessage({action = "chipmode", chipmode = chipmode})
  end)


  RegisterNetEvent('warp-hud:showharness')
  AddEventHandler('warp-hud:showharness', function(pHarnessDur)
    Harness = pHarnessDur
    SendNUIMessage({action = "cinturon", cinturon = Harness})
  end)

  RegisterNetEvent('warp-hud:hideharness')
  AddEventHandler('warp-hud:hideharness', function()
    Harness = 0
    SendNUIMessage({action = "cinturon", cinturon = Harness})
  end)


RegisterNetEvent('warp-hud:hideseatbelt')
AddEventHandler('warp-hud:hideseatbelt', function()  -- this for harness ~ nicx
  seatbelt = true
  SendNUIMessage({action = "belt", belt = seatbelt})
end)

RegisterNetEvent('warp-hud:showseatbelt')
AddEventHandler('warp-hud:showseatbelt', function()  -- this for harness ~ nicx
  seatbelt = false
  SendNUIMessage({action = "belt", belt = seatbelt})
end)
...
RegisterNetEvent('warp-admin:remove_stress')
AddEventHandler('warp-admin:remove_stress', function()
  pStress = 0
  TriggerServerEvent('warp-hud:UpdateStress_SV', pStress)
end)
...
TriggerEvent('warp-minimap-menu')
end)

RegisterNetEvent('warp-minimap-menu')
AddEventHandler('warp-minimap-menu', function()
        local minimapmaybe = {
            {
                title = "Circle Map",
                description = "Your Preference",
                action = "warp-circlemap",
                icon = "circle"
            },
            {
                title = "Square Map",
                description = "Your Preference",
                action = "warp-squaremap",
                icon = "square"
            },
        }
        exports["warp-textui"]:showContextMenu(minimapmaybe)
end)



RegisterInterfaceCallback('warp-circlemap', function(data, cb)
...
RegisterInterfaceCallback('warp-squaremap', function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNUICallback('change_wallpaper', function(data)
  TriggerEvent("warp-phone:wallpaper_alter", data.wallpaperURL)
