...
RegisterNetEvent('warp-mdt:RequestOpen')
AddEventHandler('warp-mdt:RequestOpen', function()
	TriggerServerEvent('warp-mdt:OpenMDT')
end)
...
--         TriggerEvent('warp-mdt:animation')
--         local PlayerInfo = RPC.execute("warp-mdt:getInfo")
...
--         SendNUIMessage({type = "data", name = "Welcome, " ..PlayerInfo.firstname..' '..PlayerInfo.lastname, location = playerStreetsLocation, fullname = PlayerInfo.firstname..' '..PlayerInfo.lastname})
    ...
    function EnableGUI(enable)
        if enable then 
            TriggerServerEvent('warp-mdt:opendashboard')
            SendNUIMessage({type = "bulletin", data = RPC.execute("warp-mdt:dashboardbulletin")})
            SendNUIMessage({type = "dispatchmessages"})
            SendNUIMessage({type = "warrants", data = RPC.execute("warp-mdt:getWarrants")})
        end
    ...
    -- RegisterNetEvent('warp-mdt:getActiveUnits')
    -- AddEventHandler('warp-mdt:getActiveUnits', function(lspd, bcso, sasp, ems)
    --     SendNUIMessage({ type = "getActiveUnits", lspd = lspd, bcso = bcso, sasp = sasp, ems = ems })
    -- end)
    AddEventHandler('warp-mdt:animation', function()
        if not isOpen then 
            return 
        end
    ...
    RegisterNUICallback("deleteBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("warp-mdt:deleteBulletin", data.id)
    ...
    RegisterNUICallback("updateLicence", function(data, cb)
        ...
        local cid = data.cid
        RPC.execute("warp-mdt:updateLicence", type, status, cid)
    end)
    RegisterNUICallback("newBulletin", function(data, cb)
        local ignoreId, sentData, job = RPC.execute("warp-mdt:newBulletin", data.title, data.info, data.time)
        if ignoreId == GetPlayerServerId(PlayerId()) then 
            return 
        end
        if job == "police" or job == "state" or job == "sheriff" and exports['isPed']:isPed("myjob") == "police" or exports['isPed']:isPed("myjob") == "state" or exports['isPed']:isPed("myjob") == "sheriff" then
    ...
    RegisterNetEvent('warp-mdt:open')
    AddEventHandler('warp-mdt:open', function(job, lastname, firstname)
        open = true
        EnableGUI(open)
        local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))
        local currentStreetHash, intersectStreetHash = GetStreetNameAtCoord(x, y, z)
    ...
    RegisterNUICallback("searchProfiles", function(data, cb)
        SendNUIMessage({type = "profiles", data = RPC.execute("warp-mdt:searchProfile", data.name), isLimited = RPC.execute("warp-mdt:isLimited")})
    end)
    RegisterNUICallback("saveProfile", function(data, cb)
        RPC.execute("warp-mdt:saveProfile", data.pfp, data.description, data.id)
    end)
    RegisterNUICallback("newWeapon", function(data, cb)
        RPC.execute("warp-mdt:addWeapon", data.id, data.serialnumber)
    end)
    RegisterNUICallback("getProfileData", function(data, cb)
        local isLimited = RPC.execute("warp-mdt:isLimited")
        local ProfileData = RPC.execute("warp-mdt:getProfileData", data.id)
    ...
    RegisterNUICallback("newTag", function(data, cb)
        if data.id ~= "" and data.tag ~= "" then
            RPC.execute("warp-mdt:newTag", data.id, data.tag)
        end
    end)
    RegisterNUICallback("removeProfileTag", function(data, cb)
        RPC.execute("warp-mdt:removeProfileTag", data.cid, data.text)
    end)
    RegisterNUICallback("addGalleryImg", function(data, cb)
        RPC.execute("warp-mdt:addGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("removeGalleryImg", function(data, cb)
        RPC.execute("warp-mdt:removeGalleryImg", data.cid, data.URL)
    end)
    RegisterNUICallback("searchIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("warp-mdt:searchIncidents", data.incident)})
    end)
    RegisterNUICallback("getIncidentData", function(data, cb)
        local sentData, sentConvictions = RPC.execute("warp-mdt:getIncidentData", data.id)
    ...
    RegisterNUICallback("incidentSearchPerson", function(data, cb)
        SendNUIMessage({type = "incidentSearchPerson", data = RPC.execute("warp-mdt:incidentSearchPerson", data.name)})
    end)
    RegisterNUICallback("searchBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("warp-mdt:searchBolos", data.searchVal)})
    end)
    RegisterNUICallback("getAllBolos", function(data, cb)
        SendNUIMessage({type = "bolos", data = RPC.execute("warp-mdt:getAllBolos")})
    end)
    RegisterNUICallback("getAllIncidents", function(data, cb)
        SendNUIMessage({type = "incidents", data = RPC.execute("warp-mdt:getAllIncidents")})
    end)
    RegisterNUICallback("getBoloData", function(data, cb)
        SendNUIMessage({type = "boloData", data = RPC.execute("warp-mdt:getBoloData", data.id)})
    end)
    RegisterNUICallback("jailPlayer", function(data, cb)
        RPC.execute("warp-mdt:JailPlayer", data.cid, data.time)
    end)
    RegisterNUICallback("newBolo", function(data, cb)
        local sentData = RPC.execute("warp-mdt:newBolo", data.existing, data.id, data.title, data.plate, data.owner, data.individual, data.detail, data.tags, data.gallery, data.officers, data.time)
        SendNUIMessage({type = "boloComplete", data = sentData})
    end)
    RegisterNUICallback("deleteBolo", function(data, cb)
        RPC.execute("warp-mdt:deleteBolo", data.id)
    end)
    RegisterNUICallback("deleteIncident", function(data, cb)
        RPC.execute("warp-mdt:deleteIncident", data.id, data.time)
    end)
    RegisterNUICallback("deleteReport", function(data, cb)
        RPC.execute("warp-mdt:deleteReport", data.id, data.time)
    end)
    RegisterNUICallback("deleteICU", function(data, cb)
        RPC.execute("warp-mdt:deleteICU", data.id)
    end)
    RegisterNUICallback("getAllReports", function(data, cb)
        local asdsadassa = RPC.execute("warp-mdt:getAllReports")
        print(json.encode(asdsadassa))
        SendNUIMessage({type = "reports", data = RPC.execute("warp-mdt:getAllReports")})
    end)
    RegisterNUICallback("getReportData", function(data, cb)
        SendNUIMessage({type = "reportData", data = RPC.execute("warp-mdt:getReportData", data.id)})
    end)
    RegisterNUICallback("searchReports", function(data, cb)
        SendNUIMessage({type = "reports", data = RPC.execute("warp-mdt:searchReports", data.name)})
    end)
    RegisterNUICallback("newReport", function(data, cb)
        local sentData = RPC.execute("warp-mdt:newReport", data.existing, data.id, data.title, data.type, data.detail, data.tags, data.gallery, data.officers, data.civilians, data.time)
        SendNUIMessage({type = "reportComplete", data = sentData})
    end)
    RegisterNUICallback("searchVehicles", function(data, cb)
        local sentData = RPC.execute("warp-mdt:searchVehicles", data.name)
        for i = 1, #sentData do
    ...
    RegisterNUICallback("searchWeapon", function(data, cb)
        local sentData = RPC.execute("warp-mdt:searchWeapon", data.name)
        SendNUIMessage({type = "searchedWeapons", data = sentData})
    end)
    RegisterNUICallback("getWeaponData", function(data, cb)
        local sentData = RPC.execute("warp-mdt:getWeaponData", data.serialnumber)
        SendNUIMessage({type = "getWeaponData", data = sentData})
    end)
    RegisterNUICallback("getVehicleData", function(data, cb)
        local sentData = RPC.execute("warp-mdt:getVehicleData", data.plate)
        if sentData then
    ...
    RegisterNUICallback("saveVehicleInfo", function(data, cb)
        RPC.execute("warp-mdt:saveVehicleInfo", data.plate, data.imageurl, data.notes)
    end)
    RegisterNUICallback("saveWeaponInfo", function(data, cb)
        RPC.execute("warp-mdt:saveWeaponInfo", data.serialnumber, data.imageurl, data.brand, data.type, data.notes)
    end)
    RegisterNUICallback("knownInformation", function(data, cb)
        RPC.execute("warp-mdt:knownInformation", data.type, data.status, data.plate)
    end)
    RegisterNUICallback("getAllLogs", function(data, cb)
        SendNUIMessage({type = "getAllLogs", data = RPC.execute("warp-mdt:getAllLogs")})
    end)
    RegisterNUICallback("getPenalCode", function(data, cb)
        local titles, penalcode = RPC.execute("warp-mdt:getPenalCode")
        SendNUIMessage({type = "getPenalCode", titles = titles, penalcode = penalcode})
    end)
    -- RegisterNUICallback("missingCitizen", function(data, cb)
    --     RPC.execute("warp-mdt:missingCitizen", data.cid, data.bool)
    -- end)
    ...
    --     RPC.execute("warp-mdt:deleteMissing", data.id, data.time)
    -- end)
    -- RegisterNUICallback("searchMissing", function(data, cb)
    --     local sentData = RPC.execute("warp-mdt:searchMissing", data.name)
    --     SendNUIMessage({type = "searchedMissing", data = sentData})
    -- end)
    -- RegisterNUICallback("getMissingData", function(data, cb)
    --     local sentData = RPC.execute("warp-mdt:getMissingData", data.id)
    --     SendNUIMessage({type = "getMissingData", data = sentData})
    -- end)
    -- RegisterNUICallback("saveMissingInfo", function(data, cb)
    --     RPC.execute("warp-mdt:saveMissingInfo", data.id, data.imageurl, data.notes)
    -- end)
    RegisterNUICallback("toggleDuty", function(data, cb)
        RPC.execute("warp-mdt:toggleDuty", data.cid, data.status)
    end)
    RegisterNUICallback("setCallsign", function(data, cb)
        TriggerServerEvent('warp-mdt:setCallsign', data.cid, data.newcallsign)
    end)
    RegisterNUICallback("saveIncident", function(data, cb)
        RPC.execute("warp-mdt:saveIncident", data.ID, data.title, data.information, data.tags, data.officers, data.civilians, data.evidence, data.associated, data.time)
    end)
    RegisterNUICallback("removeIncidentCriminal", function(data, cb)
        RPC.execute("warp-mdt:removeIncidentCriminal", data.cid, data.incidentId)
    end)
    RegisterNUICallback("setWaypoint", function(data, cb)
        local callInformation = RPC.execute("warp-mdt:setWaypoint", data.callid)
    ...
    RegisterNUICallback("callDetach", function(data, cb)
        local callid, sentData = RPC.execute("warp-mdt:callDetach", data.callid)
    ...
    RegisterNUICallback("removeCall", function(data, cb)
        local src, callid = RPC.execute("warp-mdt:removeCall", data.callid)
    ...
    RegisterNUICallback("callAttach", function(data, cb)
        local callid, sentData = RPC.execute("warp-mdt:callAttach", data.callid)
        local job = exports['isPed']:isPed("myjob")
        if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    RegisterNUICallback("attachedUnits", function(data, cb)
        local sentData, callid = RPC.execute("warp-mdt:attachedUnits", data.callid)
        SendNUIMessage({type = "attachedUnits", data = sentData, callid = callid})
    end)
    RegisterNUICallback("callDispatchDetach", function(data, cb)
        RPC.execute("warp-mdt:callDispatchDetach", data.callid, data.cid)
    end)
    RegisterNUICallback("setDispatchWaypoint", function(data, cb)
        local callInformation = RPC.execute("warp-mdt:setDispatchWaypoint", data.callid)
        SetNewWaypoint(callInformation['origin']['x'], callInformation['origin']['y'])
    end)
    RegisterNUICallback("callDragAttach", function(data, cb)
        RPC.execute("warp-mdt:callDragAttach", data.callid, data.cid)
    end)
    RegisterNUICallback("setWaypointU", function(data, cb)
        local sentData = RPC.execute("warp-mdt:setWaypoint:unit", data.cid)
        SetNewWaypoint(sentData.x, sentData.y) 
    end)
    -- RegisterNUICallback("dispatchMessage", function(data, cb)
    --     local sentData = RPC.execute("warp-mdt:sendMessage", data.message, data.time)
    --     local job = exports['isPed']:isPed("myjob")
    --     if job == "police" or job == "state" or job == "sheriff" or job == 'ems' then
    ...
    --     SendNUIMessage({type = "dispatchmessages", data = RPC.execute("warp-mdt:refreshDispatchMsgs")})
    -- end)
-- end
