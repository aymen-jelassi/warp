...
-- RegisterServerEvent('warp-dispatch:GrabRadioChannel')
-- AddEventHandler("warp-dispatch:GrabRadioChannel", function(radio)
-- 	local src = source
...
RegisterServerEvent('warp-mdt:OpenMDT')
AddEventHandler("warp-mdt:OpenMDT", function()
	local src = source
	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	if userjob == "police" or userjob == "sheriff" or userjob == "state" or userjob == "ems" or userjob == "judge" or userjob == "district_attorney" or userjob == "public_defender" then
        TriggerClientEvent('warp-mdt:open', src, userjob, char.last_name, char.first_name)
        TriggerClientEvent('warp-voice:getRadioChannel', src)
	end
end)

-- RegisterServerEvent("warp-mdt:opendashboard")
-- AddEventHandler("warp-mdt:opendashboard", function()
--     local src = source
-- 	UpdateUnits(src)	
-- end) 
...
LoadDreamsVersion = function()
    GetUserByCid = function(cid)
        for index, value in pairs(GetPlayers()) do
            local Player = exports["warp-base"]:getModule("Player"):GetUser(tonumber(value))
    ...
    GetCharData = function(source)
        local Player = exports["warp-base"]:getModule("Player"):GetUser(source)
        local char = Player:getCurrentCharacter()
    ...
    RPC.register("warp-mdt:getInfo", function()
        return {firstname = GetCharData(source).first_name, lastname = GetCharData(source).last_name}
    end)
    RPC.register("warp-mdt:dashboardbulletin", function()
        return GetAllBulletinData()
    end)
    -- RPC.register("warp-mdt:dashboardMessages", function()
    --     return GetAllMessages()
    -- end)
    RPC.register("warp-mdt:getWarrants", function()
        local WarrantData = {}
    ...
    --     for k, v in pairs(GetPlayers()) do
    --     local user = exports["warp-base"]:getModule("Player"):GetUser(tonumber(v))
    --         if user then
    --             local userjob = user:getVar("job")
    ...
    --         end
    --     end	
    --     TriggerClientEvent("warp-mdt:getActiveUnits", src, lspd, bcso, sasp, ems)
    -- end	    
    ...
    RPC.register("warp-mdt:getProfileData", function(identifier)
        local ReturnData = {}
        local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = identifier})
    ...
    RPC.register("warp-mdt:JailPlayer", function(cid, time)
        JailPlayer(source, cid, time)
    end)
    RPC.register("warp-mdt:saveProfile", function(profilepic, information, cid)
    ...
    RPC.register("warp-mdt:addGalleryImg", function(cid, url)
    ...
    RPC.register("warp-mdt:addWeapon", function(cid, serialnumber)
    ...
    RPC.register("warp-mdt:removeGalleryImg", function(cid, url)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewGallery = {}
    ...
    RPC.register("warp-mdt:newTag", function(identifier, tag)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = identifier
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("warp-mdt:removeProfileTag", function(cid, tagtext)
        SQL("SELECT * FROM `characters` WHERE id = @id", {
            ['@id'] = cid
        }, function(result)
            local NewTags = {}
    ...
    RPC.register("warp-mdt:updateLicence", function(type, status, cid)
        local src = source
        local user = exports["warp-base"]:getModule("Player"):GetUser(src)	
        local char = user:getCurrentCharacter()
    ...
    RPC.register("warp-mdt:deleteBulletin", function(id, time)
        local src = source
        local Player = exports["warp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("warp-mdt:newBulletin", function(title, info, time)
        local src = source
        local Player = exports["warp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("warp-mdt:searchProfile", function(name)
        local Matches = {}
    ...
    RPC.register("warp-mdt:searchIncidents", function(incident)
        local Matches = {}
    ...
    RPC.register("warp-mdt:saveIncident", function(ID, title, information, tags, officers, civilians, evidence, associated, time)
        local src = source
    ...
    RPC.register("warp-mdt:getAllIncidents", function()
        local Tables = {}
    ...
    RPC.register("warp-mdt:getIncidentData", function(id)
        local result = SQL('SELECT * FROM mdt_incidents WHERE id = @id', {['@id'] = id})
        local convictions = {}
    ...
    RPC.register("warp-mdt:incidentSearchPerson", function(name)
        local Matches = {}
        local users = SQL("SELECT * FROM characters", {})
        for index = 1, #users, 1 do
            local firstname = string.lower(users[index].first_name)
    ...
    RPC.register("warp-mdt:removeIncidentCriminal", function(cid, incidentId)
    ...
    RPC.register("warp-mdt:getPenalCode", function()
        return Config['OffensesTitels'], Config["Offenses"]
    end)
    -- RPC.register("warp-mdt:missingCitizen", function(cid, time)
    --     local result = SQL('SELECT * FROM characters WHERE id = @id', {['@id'] = cid})
    ...
    --     end
    -- end)
    -- RPC.register("warp-mdt:getAllMissing", function()
    --     local Tables = {}
    ...
    --     return Tables
    -- end)
    -- RPC.register("warp-mdt:deleteMissing", function(id, time)
    --     local src = source
    ...
    -- end)
    -- RPC.register("warp-mdt:saveMissingInfo", function(id, imageurl, notes)
    ...
    --     })
    -- end)
    -- RPC.register("warp-mdt:searchMissing", function(name)
    --     local ReturnData = {}
    ...
    --     return ReturnData
    -- end)
    -- RPC.register("warp-mdt:getMissingData", function(id)
    ...
    --     end
    -- end)
    RPC.register("warp-mdt:searchBolos", function(searchVal)
        local Matches = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("warp-mdt:getAllBolos", function()
        local Tables = {}
        local BolosData = SQL("SELECT * FROM mdt_bolos", {})
        for index = 1, #BolosData, 1 do
    ...
    RPC.register("warp-mdt:getBoloData", function(id)
        local result = SQL('SELECT * FROM mdt_bolos WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("warp-mdt:newBolo", function(existing, id, title, plate, owner, individual, detail, tags, gallery, officers, time)
        local src = source
        local Player = exports["warp-base"]:getModule("Player"):GetUser(src)
    ...
    RPC.register("warp-mdt:deleteBolo", function(id, time)
        local src = source
    ...
    RPC.register("warp-mdt:deleteReport", function(id, time)
        local src = source
    ...
    RPC.register("warp-mdt:deleteIncident", function(id, time)
        local src = source
    ...
    RPC.register("warp-mdt:deleteICU", function(id)
        SQL("DELETE FROM mdt_bolos WHERE id = @id", {['@id'] = id})
    end)
    RPC.register("warp-mdt:getAllReports", function()
        local Tables = {}
    ...
    RPC.register("warp-mdt:getReportData", function(id)
        local result = SQL('SELECT * FROM mdt_report WHERE id = @id', {['@id'] = id})
        return result[1]
    end)
    RPC.register("warp-mdt:searchReports", function(name)
        local Matches = {}
    ...
    RPC.register("warp-mdt:newReport", function(existing, id, title, reporttype, detail, tags, gallery, officers, civilians, time)
        local src = source
        if not existing then
    ...
    RPC.register("warp-mdt:searchVehicles", function(name)
        local ReturnData = {}
    ...
    RPC.register("warp-mdt:searchWeapon", function(name)
        local ReturnData = {}
    ...
    RPC.register("warp-mdt:getWeaponData", function(serialnumber)
        local result = SQL('SELECT * FROM weapons_serials WHERE serial = @serial', {['@serial'] = serialnumber})
        if result[1] then
            return result[1]
        end
    end)
    RPC.register("warp-mdt:saveWeaponInfo", function(serialnumber, imageurl, brand, type, notes)
    ...
    RPC.register("warp-mdt:getVehicleData", function(plate)
    ...
    RPC.register("warp-mdt:saveVehicleInfo", function(plate, imageurl, notes)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
    ...
    RPC.register("warp-mdt:knownInformation", function(type, status, plate)
        local result = SQL('SELECT * FROM mdt_vehicleinfo WHERE plate = @plate', {['@plate'] = plate})
        if result[1] then
            if type == "code5" then
    ...
    RPC.register("warp-mdt:getAllLogs", function()
        local MDTLogs = {}
        return MDTLogs
    end)
    RPC.register("warp-mdt:toggleDuty", function(cid, status)
        local src = source
        ChangePlayerDuty(src, status)
    end)
    RegisterServerEvent("warp-mdt:setCallsign", function(cid, callsign)
    ...
    RPC.register("warp-mdt:callDetach", function(callid)
        local src = source
        local NewTable = {}
    ...
    RPC.register("warp-mdt:removeCall", function(callid)
        MDTDispatchData[callid] = nil
        return source, callid
    end)
    RPC.register("warp-mdt:callAttach", function(callid)
        local src = source
    ...
    RPC.register("warp-mdt:attachedUnits", function(callid)
        local ReturnData = {}
        for index, data in pairs(AttachedUnits) do
            if data.CallId == callid then
                local Player = exports["warp-base"]:getModule("Player"):GetUser(data.Source)
                local char = Player:getCurrentCharacter()
                table.insert(ReturnData, {
                    cid = char.id,
                    job = Player.job,
    ...
    RPC.register("warp-mdt:callDispatchDetach", function(callid, cid)
    ...
    RPC.register("warp-mdt:setDispatchWaypoint", function(callid)
        return MDTDispatchData[callid]
    end)
    RPC.register("warp-mdt:callDragAttach", function(callid, cid)
        local Player = GetUserByCid(tostring(cid))
        for _, data in pairs(AttachedUnits) do
            if data.CallId == callid then
    ...
    -- RPC.register("warp-mdt:sendMessage", function(message, time)
    --     local src = source
    --     local Player = exports["warp-base"]:getModule("Player"):GetUser(src)
    --     local char = Player:getCurrentCharacter()
    --     local ReturnData = {}
    ...
    --     return ReturnData
    -- end)
    -- RPC.register("warp-mdt:refreshDispatchMsgs", function()
    --     return GetAllMessages()
    -- end)
    RPC.register("warp-mdt:setWaypoint:unit", function(cid)
        local Player = GetUserByCid(cid)
        local PlayerCoords = GetEntityCoords(GetPlayerPed(Player.source))
        return PlayerCoords
    end)
    RPC.register("warp-mdt:isLimited", function()
        local Player = exports["warp-base"]:getModule("Player"):GetUser(source)
