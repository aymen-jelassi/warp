...
RegisterNUICallback("order", function(data)
    ...
    TriggerServerEvent('warp-blackmarket:makeorder', data)
end)
...
RegisterNetEvent("warp-blackmarket:open")
AddEventHandler("warp-blackmarket:open",function()
    if exports['warp-inventory']:hasEnoughOfItem('laptop', 1) then
        local finished = exports["warp-taskbar"]:taskBar(5000,"Plugging in usb")
        Animation()
        open(true)
    ...
    while BlackMarketItems do
        ...
        if distance < 5.0 and not inVehicle then
            ...
            if IsControlJustReleased(1,38) and distance < 1.5 then
                if exports['warp-inventory']:hasEnoughOfItem('usbdevice', 1) and exports['warp-inventory']:hasEnoughOfItem('laptop', 1) then
                    ClearBlips()  
