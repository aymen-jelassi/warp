...
AddEventHandler('veh.examine', function(plate)
    local src = source
    if plate ~= nil then
        exports.oxmysql:execute('SELECT * FROM characters_cars WHERE license_plate = ?', {plate}, function(result) 
...
AddEventHandler('veh.callDegredation', function(plate)
    local _src = source
    exports.oxmysql:execute('SELECT * FROM characters_cars WHERE license_plate = @plate', {['@plate'] = plate}, function(data) 
        if data[1] then
...
AddEventHandler('veh.updateVehicleHealth', function(tempReturn)
    ...
    exports.oxmysql:execute("UPDATE characters_cars SET engine_damage = @engine_damage, body_damage = @body_damage, fuel = @fuel WHERE license_plate = @plate", {
...
AddEventHandler('veh.updateVehicleDegredationServer', function(plate,br,ax,rad,cl,tra,elec,fi,ft)
    exports.oxmysql:execute('SELECT license_plate FROM characters_cars WHERE license_plate = @plate', {['@plate'] = plate}, function (result)
        if result[1] ~= nil then
