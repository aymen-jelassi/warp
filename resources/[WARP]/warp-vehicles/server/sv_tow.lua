...
RegisterServerEvent("warp-vehicles:checkrepo")
AddEventHandler("warp-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("warp-vehicles:repo")
-- AddEventHandler("warp-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("warp-vehicles:repo:success", src)
...
-- RegisterServerEvent("warp-vehicles:judgerepo")
-- AddEventHandler("warp-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("warp-vehicles:judgerepodelete")
-- AddEventHandler("warp-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("warp-vehicles:repo2")
-- AddEventHandler("warp-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["warp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("warp-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("warp-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("warp-vehicles:release:vehicle")
-- AddEventHandler("warp-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['warp-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("warp-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)