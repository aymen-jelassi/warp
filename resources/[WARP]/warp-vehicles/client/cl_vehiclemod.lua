...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        disableTurning()
        local finished = exports["warp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
        if finished == 100 then
            ...
            TriggerEvent('warp-hud:hideharness')
        end
...
function carCrash()
    endNos()
    local new_health = GetVehicleEngineHealth(currentVehicle) - math.random(100, 200)
...
function nosInit()
    ...
    if nosCar ~= 0 and nosCar ~= false and nosCar ~= nil then
        Citizen.CreateThread(function()
            ...
            local burst = math.ceil( (nosForce + nosForce * 1.0) * 100000 ) / 100000
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["warp-taskbar"]:taskBar(5000,"Taking off Harness",true)
        TriggerEvent('warp-hud:hideharness')
        TriggerEvent('warp-hud:showseatbelt')
    else
        finished = exports["warp-taskbar"]:taskBar(5000,"Putting on Harness",true)
        TriggerEvent('warp-hud:showharness')
        TriggerEvent('warp-hud:hideseatbelt')
    end
    if (finished == 100) then
...
RegisterNetEvent("warp-vehicledegrade:fetch:nos:cl", function(pPlate)
    if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
        if driverPed == playerPed then
            TriggerServerEvent("warp-vehicledegrade:fetch:nos", GetVehicleNumberPlateText(currentVehicle))
        end
    end
end)

RegisterNetEvent("warp-vehicledegrade:fetch:cl", function(pPlate)
    if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
        if driverPed == playerPed then
            NosVehicles[currentVehicle] = pPlate
...
AddEventHandler('NosStatus', function()
    ...
    if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
        if driverPed == playerPed then
            ...
            TriggerServerEvent("warp-vehicledegrade:apply:nos", GetVehicleNumberPlateText(currentVehicle))
        end
    end
end)
...
AddEventHandler("vehicleMod:useHarnessItem",function()
    local c_veh_plate = GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false))
    local isActive = exports['warp-garages']:fake("active", c_veh_plate)
    if isActive then
        local real_plate = exports['warp-garages']:fake("real_plate", c_veh_plate)
        TriggerServerEvent("vehicleMod:applyHarness", real_plate, 1.0)
    else
        ...
        if tempVehicle ~= currentVehicle then
            ...
            if currentVehicle == nil or currentVehicle == 0 or currentVehicle == false then
                ...
                if driverPed == playerPed then
                    TriggerEvent("DensityModifierEnable", true)
                    TriggerEvent("noshud", NosVehicles[currentVehicle], false)
                    TriggerEvent("warp-vehicledegrade:fetch:nos:cl")
                    TriggerEvent("tuner:setDriver")
                else
                ...
                if not IsThisModelAHeli(vehModel) and not IsThisModelABoat(vehModel) and not IsThisModelABike(vehModel) and IsVehicleOnAllWheels(currentVehicle) and IsVehicleEngineOn(currentVehicle) then
                    if GetEntitySpeed(currentVehicle) > 12.5 and NosVehicles[currentVehicle] > 0 then
                        ...
                        elseif not disablenos then
                            ...
                            TriggerServerEvent("warp-vehicledegrade:update:nos", GetVehicleNumberPlateText(currentVehicle), pNewAmount)
                        end
                    end
                end
            end
