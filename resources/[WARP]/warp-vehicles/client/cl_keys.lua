...
AddEventHandler('event:control:npkeys', function(useID)
    ...
    if IsPedInAnyVehicle(playerped, false) then
      ...
      if not searchedVehs[veh] and not hasKey(plate) and GetPedInVehicleSeat(veh, -1) == playerped  then  
        if useID == 1 then
          Citizen.Wait(1000)
          TriggerEvent('warp-dispatch:carjacking')
          shutoffenginesearch()
        elseif useID == 2 then
          Citizen.Wait(1000)
...
function shutoffenginesearch()
    ...
    if not fuckingRETARDED then
       ...
       fuckingRETARDED = true
        ...
        local finished = exports["warp-taskbar"]:taskBar(5000,"Searching")
        Citizen.Wait(100)
        ...
        if luck == 69 and finished == 100 then
          local finished = exports["warp-taskbar"]:taskBar(2000,"Found and Using Keys")
        ...
        local finished = exports["warp-taskbar"]:taskBar(5000,"Searching Backseat")
        if luck > 75 and finished then
          if not IsPedInAnyVehicle(PlayerPedId(), false) then
            fuckingRETARDED = false
            return
          end
          local finished = exports["warp-taskbar"]:taskBar(2000,"Found Content")
          DropItemPed()
        end
...
function shutoffenginehotwire()
    ...
    if not fuckingRETARDED then
        ...
        local finished = exports["warp-taskbar"]:taskBar(carTimer,"Attempting Hotwire")
        Citizen.Wait(100)
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                TriggerEvent('warp-dispatch:carjacking')
                local finished = exports["warp-taskbar"]:taskBar(3000,"Taking Keys",false)
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      TriggerEvent('warp-dispatch:carjacking')
                      local finished = exports["warp-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
                      else
