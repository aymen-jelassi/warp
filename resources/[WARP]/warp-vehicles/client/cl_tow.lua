...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["warp-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
			
				if targetVehicle ~= 0 then
					TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
					local part2 = exports["warp-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["warp-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["warp-taskbar"]:taskBar(7000,"Unhooking Vehicle")
...
RegisterNetEvent("warp-vehicles:repo:success")
AddEventHandler("warp-vehicles:repo:success", function()
...
RegisterNetEvent("warp-vehicles:repo:success2")
AddEventHandler("warp-vehicles:repo:success2", function()
	local veh = GetVehiclePedIsIn(PlayerPedId())
	if veh ~= 0 then
		DeleteVehicle(veh)
		DeleteEntity(veh)
...
AddEventHandler("search:list:repo", function()
	...
	if job == 'pdm' or job == 'towunion' or job == 'sanders' then
        ...
        if valid then
            TriggerServerEvent("warp-vehicles:checkrepo", valid[1].input)
        end
	else
...
AddEventHandler("search:list:showJudgeRepo", function()
	...
	if job == 'judge' then
        ...
        if valid then
            TriggerServerEvent("warp-vehicles:judgerepo", valid[1].input)
        end
	else
		TriggerEvent('DoLongHudText', 'You cannot do this!', 2)
	end
...
RegisterNetEvent("warp-vehicles:towBill")
AddEventHandler("warp-vehicles:towBill", function(amount)
