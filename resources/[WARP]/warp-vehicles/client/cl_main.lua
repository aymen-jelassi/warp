...
AddEventHandler("mech:tools", function(material, amount)
    ...
    if exports["warp-inventory"]:hasEnoughOfItem(material, amount,false) then
...
AddEventHandler("mech:tools:cl2", function(input, input2)
    ...
    if job >= 1 then
        ...
        if amount <= 10 then
            ...
            local finished = exports["warp-taskbar"]:taskBar(35000,"Repairing")
...
AddEventHandler('towgarage:repairamount', function(data)
	...
	if #(vector3(841.25274658203,-974.61096191406,26.482788085938) - GetEntityCoords(PlayerPedId())) < 35 then
		if job >= 1 then
			local keyboard = exports["warp-applications"]:KeyboardInput({
				header = "Repair: ".. part,
				rows = {
				{
					id = 0, 
...
RegisterNetEvent('warp-towjob:StoreMaterialsMain')
AddEventHandler('warp-towjob:StoreMaterialsMain', function()
...
AddEventHandler("mech:check:internal:storage", function()
    local job = exports["isPed"]:GroupRank('tuner_shop')
    if job >= 1 then
        TriggerServerEvent("mech:check:materials", 'tuner_shop')
    else
