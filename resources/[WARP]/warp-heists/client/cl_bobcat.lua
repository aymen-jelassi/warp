...
RegisterNetEvent('warp-bobcat:firstdoor')
AddEventHandler('warp-bobcat:firstdoor', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(882.17254638672, -2258.0190429688, 32.461284637451) - player)
    if distance < 1.5 then
        if exports["warp-duty"]:LawAmount() >= 0 then
            if exports["warp-inventory"]:hasEnoughOfItem("thermal_charge", 1) then
                ...
                exports['warp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('warp-doors:change-lock-state', 273, false) 
                        TriggerServerEvent("warp-doors:change-lock-state", 274, false)
                        TriggerEvent('warp-dispatch:bobcat_robbery')
                        Citizen.Wait(3000)
...
RegisterNetEvent('warp-bobcat:seconddoor')
AddEventHandler('warp-bobcat:seconddoor', function()
local player = GetEntityCoords(PlayerPedId())
local distance = #(vector3(880.89782714844, -2263.880859375, 32.441692352295) - player)
if distance < 1.5 then
    if exports["warp-duty"]:LawAmount() >= 0 then
            if exports["warp-inventory"]:hasEnoughOfItem("Gruppe6Card2", 1, false) and exports["warp-inventory"]:hasEnoughOfItem("thermal_charge", 1) then
                    insidedoor()
                    TriggerEvent('warp-hud:show_hackerman')
                    TriggerEvent("client:newStress",true, 75)
                        exports['warp-thermite']:OpenThermiteGame(function(success)
                            if success then
                                ...
                                TriggerEvent("inventory:removeItem", "thermal_charge", 1)
                                --Door IDS
                                TriggerServerEvent("warp-doors:change-lock-state", 275, false)
                                TriggerEvent('warp-dispatch:bobcat_robbery')
                                Citizen.Wait(3000)
                            else
                                Citizen.Wait(2000)
...
RegisterNetEvent("bobcat:success:panel", function()
    TriggerEvent('DoLongHudText', "Success!", 1)
    TriggerServerEvent("warp-doors:change-lock-state", 275, false)
    TriggerServerEvent("warp-doors:change-lock-state", 276, false)
    TriggerEvent('warp-dispatch:bobcat_robbery')
end)
...
RegisterNetEvent("bobcat2:success:panel", function()
    TriggerEvent('DoLongHudText', "Success!", 1)
    TriggerServerEvent("warp-doors:change-lock-state", 277, false)
    TriggerEvent('warp-dispatch:bobcat_robbery')
end)
...
RegisterNetEvent('warp-bobcat:bubbles')
...
AddEventHandler('lumo:client:requestblast', function()
	TriggerServerEvent("warp-bobcat:request_door")
end)
...
AddEventHandler('lumo:pedbomb', function()
    ...
    TriggerServerEvent("warp-bobcat:bubbles")
    TriggerEvent("lumo:propcreate")
    Wait(1500)
...
RegisterNetEvent('warp-bobcat:search_crate_1unchecked') -- PED WALK
AddEventHandler('warp-bobcat:search_crate_1unchecked', function()
    TriggerServerEvent("warp-bobcat:search_crate_1")
end)

RegisterNetEvent('warp-bobcat:search_crate_2unchecked') -- PED WALK
AddEventHandler('warp-bobcat:search_crate_2unchecked', function()
    TriggerServerEvent("warp-bobcat:search_crate_2")
end)

RegisterNetEvent('warp-bobcat:search_crate_3unchecked') -- PED WALK
AddEventHandler('warp-bobcat:search_crate_3unchecked', function()
    TriggerServerEvent("warp-bobcat:search_crate_3")
end)

RegisterNetEvent('warp-bobcat:search_crate_1') 
AddEventHandler('warp-bobcat:search_crate_1', function()
    FreezeEntityPosition(GetPlayerPed(-1),true)
    TriggerEvent('warp-hud:show_money')
    TriggerEvent("doAnim","search")
    TriggerEvent('warp-dispatch:bobcat_robbery')
    local finished = exports["warp-taskbar"]:taskBar(30000,"Looting")
    if finished == 100 then
        TriggerEvent('warp-heists:bobcat_loot')
...
RegisterNetEvent('warp-bobcat:search_crate_2') 
...
RegisterNetEvent('warp-bobcat:search_crate_3')
...
RegisterNetEvent('warp-heists:bobcat_loot')
AddEventHandler('warp-heists:bobcat_loot', function()
