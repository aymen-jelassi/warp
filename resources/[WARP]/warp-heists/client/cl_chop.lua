...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=true,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("warp-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == DreamsChopCar then
        ...
        DreamsStartChopping()
        exports['warp-textui']:showInteraction('[E] Start Chopping')
    end
end)

function DreamsStartChopping()
	Citizen.CreateThread(function()
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["warp-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["warp-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["warp-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
    ...
    TriggerEvent('dreams-chop:cooldown')
    return success
end

RegisterNetEvent('dreams-chop:cooldown')
AddEventHandler('dreams-chop:cooldown', function()
    CoolDownActive = true
    Citizen.Wait(900000)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("warp-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('warp-chopshop:clock-in')
AddEventHandler('warp-chopshop:clock-in', function()
    if CoolDownActive then
...
RegisterNetEvent('warp-chopshop:clock-out')
AddEventHandler('warp-chopshop:clock-out', function()
    if ClockedIntoChop then
...
function DreamsCreateChopCar()
	if DoesEntityExist(DreamsChopCar) then
	    SetVehicleHasBeenOwnedByPlayer(DreamsChopCar,false)
		SetEntityAsNoLongerNeeded(DreamsChopCar)
		DeleteEntity(DreamsChopCar)
	end

    local DreamsChopVehicle = GetHashKey(ChopCars[math.random(#ChopCars)])
    RequestModel(DreamsChopVehicle)
    while not HasModelLoaded(DreamsChopVehicle) do
        Citizen.Wait(0)
    end
    ...
    DreamsChopCar = CreateVehicle(DreamsChopVehicle, x, y, z, h, true, false)

	local pos = GetEntityCoords(DreamsChopCar, false)
    Citizen.CreateThread(function()
        while true do
          Citizen.Wait(5)
          if GetVehiclePedIsIn(PlayerPedId(), false) == DreamsChopCar then
                SetNewWaypoint(1360.2725830078,-2095.0549316406,51.993408203125)
                TriggerEvent('phone:addJobNotify', 'Drive to the GPS and begin Chopping')
                DeleteBlip(blipPickup)
            return
          end
        end
