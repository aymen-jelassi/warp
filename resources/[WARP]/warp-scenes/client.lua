...
RegisterNetEvent('warp-scenes:send', function(sent)
    scenes = sent
end)
...
RegisterCommand('-scenecreate', function()
    ...
    local scene = exports["warp-applications"]:KeyboardInput({
        header = "Add Scene",
        rows = {
            {
                id = 0,
    ...
    TriggerServerEvent('warp-scenes:add', coords, message, color, distance)
end)
...
RegisterCommand('+scenedelete', function()
    ...
    if scene ~= nil then
        TriggerServerEvent('warp-scenes:delete', scene)
    end
end)
...
TriggerServerEvent('warp-scenes:fetch')
