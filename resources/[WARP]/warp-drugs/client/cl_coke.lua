...
function VoidCocaineDropLocation()
    TriggerEvent('warp-dispatch:unauthorised_aircraft')
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("dreams_cocaine_plane_drop_off", vector3(-2238.35, 3274.98, 32.81), 20, 21.8, {
...
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "dreams_cocaine_plane_drop_off" and DreamsRPCanDropOffPlane and GetVehiclePedIsIn(PlayerPedId(), false) == DreamsCokePlane then
        ...
        DreamsCocaineDrop()
		exports['warp-textui']:showInteraction("[E] Drop Off Plane")
    end
end)
	...
	Citizen.CreateThread(function()
        while DreamsCokePlaneDrop do
			...
			if IsControlJustReleased(0, 38) then
                local finishedcoke = exports['warp-taskbar']:taskBar(7500, 'Dropping off plane')
...
Citizen.CreateThread(function()
    exports["warp-polyzone"]:AddBoxZone("dreams_cocaine_plane_start", vector3(839.42, 2176.75, 52.29), 1, 1.2, {
...
AddEventHandler('warp-polyzone:enter', function(name)
    if name == "dreams_cocaine_plane_start" then
        ...
        DreamsCocaineStart2()
		exports['warp-textui']:showInteraction("[E] Talk")
    end
end)
	...
	TriggerEvent('warp-context:sendMenu', {
		{
			...
			params = {
				event = "warp-jobs:fishing_sell:cokebaggy",
			}
		},
		{
			id = "2",
...
AddEventHandler('sellcoke', function()
    local pSellCokeAmount = exports["warp-applications"]:KeyboardInput({
    ...
    if pSellCokeAmount[1] ~= nil then
        if exports['warp-inventory']:hasEnoughOfItem('coke5g', pSellCokeAmount[1].input) then
            FreezeEntityPosition(PlayerPedId(), true)
            local finished = exports['warp-taskbar']:taskBar(1000*pSellCokeAmount[1].input, 'Selling coke baggies')
...
--     if exports["warp-inventory"]:hasEnoughOfItem("coke5g",5,false) then
--         FreezeEntityPosition(PlayerPedId(), true)
--         local finished = exports["warp-taskbar"]:taskBar(1000,"selling coke you criminal SHAME")
--         if finished == 100 then
--             FreezeEntityPosition(PlayerPedId(), false)
